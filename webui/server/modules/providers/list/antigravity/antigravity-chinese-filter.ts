...
export function localizeEnglishThought(thought: string): string {
 ...
 */
export function humanizeAntigravityError(rawError: string): string {
  ...
  if (
    /location is not supported/i.test(text) ||
...
3. 直接在输入框发送“继续”，即可无缝恢复对话。`;
  ...
  if (
    /subscriber fell behind updates/i.test(text) ||

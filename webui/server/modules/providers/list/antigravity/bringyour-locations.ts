...
export interface BringYourLocation {
  name: string;
  code: string;
  zh: string;
  flag: string;
...
export const FALLBACK_LOCATIONS: BringYourLocation[] = [
  {
    "name": "United States",
    "code": "us",
    "zh": "美国",
...
let cache: { data: BringYourLocation[]; timestamp: number } = {
...
export async function fetchBringYourLocations(): Promise<BringYourLocation[]> {
  const now = Date.now();
  if (cache.data && (now - cache.timestamp < CACHE_TTL_MS)) {
    return cache.data;
  }

  return new Promise<BringYourLocation[]>((resolve) => {
    const req = https.get("https://api.bringyour.com/network/provider-locations", { timeout: 4000 }, (res) => {
      ...
      res.on("end", () => {
        try {
          ...
          const list: BringYourLocation[] = (json.locations as any[]).map((l: any) => {
          ...
          }).sort((a: BringYourLocation, b: BringYourLocation) => b.count - a.count);

...
export function readUrnAuth(): UrnAuthConfig {
  const fallback: UrnAuthConfig = {
    userAuth: '',
    password: '',
...
export function getProxyStatus(): UrnProxyStatus {
  ...
  let activeProviderCount: number | null = null;
...
export function restartProxy(): { ok: boolean; killed: number; message: string } {
  let killed = 0;
  try {
...
export function getProxyToggle(): 'yes' | 'no' {
...
export function resolveAntigravityProxyEnv(baseEnv: NodeJS.ProcessEnv = process.env): Record<string, string> {
...
export async function testProxyConnectivity(targetMode?: 'yes' | 'no'): Promise<{
  success: boolean;
  latencyMs: number;
  mode: 'yes' | 'no';
  message: string;

...
function tryExtractFromLog(file: string): string | null {
...
function extractAntigravitySessionName(logFile: string, fallback: string): string {
  ...
  try {
    ...
    if (brainIdx !== -1 && brainIdx + 1 < parts.length) {
      ...
      if (primary && primary !== logFile) {
        ...
        if (primaryExtracted) return primaryExtracted;
      }
    }
  } catch (_) {}
...
function findPrimaryLogFile(logsDir: string): string | null {
  ...
  if (fs.existsSync(chunk0)) return chunk0;
  const chunkFull0 = path.join(logsDir, 'chunks', 'transcript_full', '00000000.jsonl');
  if (fs.existsSync(chunkFull0)) return chunkFull0;
  return null;
}
...
export class AntigravitySessionSynchronizer implements IProviderSessionSynchronizer {
  ...
  async synchronize(_since?: Date): Promise<number> {
    ...
    if (!fs.existsSync(brainDir)) return 0;

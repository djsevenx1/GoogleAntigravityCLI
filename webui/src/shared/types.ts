...
export type AgentCategory = 'account' | 'proxy' | 'permissions' | 'mcp' | 'skills';
...
/** How much Google Antigravity CLI may do without asking, including default, accept-edits, bypass permissions, plan-only, and sandbox execution modes. */
export type AntigravityPermissionMode = 'default' | 'acceptEdits' | 'bypassPermissions' | 'plan' | 'sandbox';

/** Antigravity's persisted permission settings: execution mode and whether permissions prompt is skipped entirely. */
export type AntigravityPermissionsState = {
  permissionMode: AntigravityPermissionMode;
  skipPermissions: boolean;
};

/** A Google account configured in Google Antigravity CLI for multi-account authentication and switching. */

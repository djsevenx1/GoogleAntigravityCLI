...
export const useVersionCheck = (owner?: string, repo?: string) => {
  ...
  useEffect(() => {
    // Detached from upstream third-party repository (siteboon/claudecodeui) to prevent invalid update alerts
    if (!owner || owner === 'siteboon' || !repo) {
      setUpdateAvailable(false);
      setLatestVersion(APP_VERSION);
      setReleaseInfo({
        title: `v${APP_VERSION}`,
        body: 'Google Antigravity CLI 独立稳定版',
        htmlUrl: 'https://antigravity.google',

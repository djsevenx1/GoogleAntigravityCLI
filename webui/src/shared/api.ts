...
export const api = {
  ...
  projects: () => get('/api/projects?skipSync=1'),
  archivedProjects: () => get('/api/projects/archived?skipSync=1'),
  projectSessions: (
    projectId: string,
  ...
  providers: {
    ...
    antigravityAccounts: () => get('/api/providers/antigravity/accounts'),
    switchAntigravityAccount: (email: string) =>
      post('/api/providers/antigravity/accounts/switch', { email }),
    addAntigravityAccount: (payload: { label?: string; tokenData?: unknown }) =>
      post('/api/providers/antigravity/accounts/add', payload),
    saveCurrentAntigravityAccount: (payload: { label?: string }) =>
      post('/api/providers/antigravity/accounts/save-current', payload),
    updateAntigravityAccountLabel: (email: string, label: string) =>
      patch(`/api/providers/antigravity/accounts/${encodeURIComponent(email)}`, { label }),
    removeAntigravityAccount: (email: string) =>
      del(`/api/providers/antigravity/accounts/${encodeURIComponent(email)}`),
    startAntigravityCliLogin: () => post('/api/providers/antigravity/accounts/login/start'),
    completeAntigravityCliLogin: (id: string, code: string) =>
      post('/api/providers/antigravity/accounts/login/complete', { id, code }),
    getAntigravityCliLoginStatus: (id: string) =>
      get(`/api/providers/antigravity/accounts/login/status${query({ id })}`),
    cancelAntigravityCliLogin: (id: string) =>
      post('/api/providers/antigravity/accounts/login/cancel', { id }),
    antigravityQuota: (force?: boolean, email?: string) =>
      get(`/api/providers/antigravity/quota${query({ force, email })}`),
    antigravityQuotaSnapshot: (model?: string, force?: boolean, email?: string) =>
      get(`/api/providers/antigravity/quota/snapshot${query({ model, force, email })}`),
    getAntigravityProxyStatus: () => get('/api/providers/antigravity/proxy/status'),
    getAntigravityProxyLocations: () => get('/api/providers/antigravity/proxy/locations'),

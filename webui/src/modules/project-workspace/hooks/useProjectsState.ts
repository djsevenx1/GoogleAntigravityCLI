  ...
  if (existingIndex >= 0) {
    ...
    for (const [index, session] of sessions.entries()) {
      if (index === existingIndex) {
        ...
        const isGenericSummary = (val?: string | null): boolean => {
          if (!val) return true;
          const trimmed = val.trim();
          if (!trimmed) return true;
          return (
            /^Antigravity(\s+[\w-]+)?$/i.test(trimmed) ||
            /^Untitled\s+(Claude|Codex|Cursor|Session)/i.test(trimmed) ||
            /^OpenCode(\s+[\w-]+)?$/i.test(trimmed)
          );
        };
        // Never let a later upsert that carries an empty or generic fallback summary blank out or
        // downgrade a title we already have. Fresh sessions momentarily broadcast an empty or fallback
        // custom_name before the disk indexer fills it in, which would otherwise flicker the title.
        if (
          session.summary?.trim() &&
          !isGenericSummary(session.summary) &&
          isGenericSummary(normalizedSession.summary)
        ) {
          updated.summary = session.summary;
        } else if (!normalizedSession.summary?.trim() && session.summary?.trim()) {
          updated.summary = session.summary;
        }
...
export function useProjectsState({
  ...
  useEffect(() => {
    ...
    // 兜底保护：移动端或网络抖动时，最多等待 5 秒，超时强制解除 loading 状态，防止无限转圈

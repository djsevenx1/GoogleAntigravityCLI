...
import type { AgentProvider, AntigravityPermissionMode, AntigravityPermissionsState, ClaudePermissionsState, CodeEditorSettingsState, CodexPermissionMode, CursorPermissionsState, NotificationPreferencesState, ProjectSortOrder, SettingsMainTab } from '@/shared/types';
...
const KNOWN_MAIN_TABS: SettingsMainTab[] = ['agents', 'appearance', 'git', 'api', 'browser', 'notifications', 'plugins', 'about'];

const normalizeMainTab = (tab: string): SettingsMainTab => {
  // Keep backwards compatibility with older callers that still pass "tools" or "tasks".
...
const toAntigravityPermissionMode = (value: unknown): AntigravityPermissionMode => {
  if (value === 'acceptEdits' || value === 'bypassPermissions' || value === 'plan' || value === 'sandbox') {
    return value;
  }
...
export function useSettingsController({ isOpen, initialTab }: UseSettingsControllerArgs) {
  ...
  const [antigravityPermissions, setAntigravityPermissions] = useState<AntigravityPermissionsState>(() => ({
  ...
  const loadSettings = useCallback(async () => {
    try {
      const savedAntigravitySettings = readUserPreference<AntigravitySettingsStorage>('antigravityPermissions', {});
      const agMode = toAntigravityPermissionMode(savedAntigravitySettings.permissionMode);
      setAntigravityPermissionMode(agMode);
      setAntigravityPermissions({
        permissionMode: agMode,
        skipPermissions: Boolean(savedAntigravitySettings.skipPermissions),
      });
      ...
      setAntigravityPermissions({ permissionMode: 'default', skipPermissions: false });
  ...
  const saveSettings = useCallback(async () => {
    ...
    antigravityPermissions.skipPermissions,

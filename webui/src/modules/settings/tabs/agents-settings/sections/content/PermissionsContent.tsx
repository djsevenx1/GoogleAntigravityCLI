...
function AntigravityPermissions({
  ...
  const modes: Array<{
    ...
    {
      id: 'default',
      titleKey: 'permissions.antigravity.modes.default.title',
      descKey: 'permissions.antigravity.modes.default.description',
      flag: '交互确认 (Interactive)',
    },
    {
      id: 'acceptEdits',
      titleKey: 'permissions.antigravity.modes.acceptEdits.title',
      descKey: 'permissions.antigravity.modes.acceptEdits.description',
      flag: '--mode accept-edits',
    },
    {
      id: 'plan',
      titleKey: 'permissions.antigravity.modes.plan.title',
      descKey: 'permissions.antigravity.modes.plan.description',
      flag: '--mode plan',
    },
    {
      id: 'sandbox',
      titleKey: 'permissions.antigravity.modes.sandbox.title',
      descKey: 'permissions.antigravity.modes.sandbox.description',
      flag: '--sandbox',
    },
    {
      id: 'bypassPermissions',
      titleKey: 'permissions.antigravity.modes.bypassPermissions.title',
      descKey: 'permissions.antigravity.modes.bypassPermissions.description',
  ...
  return (
    <div className="space-y-6">
      {/* Skip Permissions warning / quick toggle */}
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          <AlertTriangle className="h-5 w-5 text-orange-500" />
          <h3 className="text-lg font-medium text-foreground">{t('permissions.title')}</h3>
        ...
        <div className="rounded-lg border border-orange-200 bg-orange-50 p-4 dark:border-orange-800 dark:bg-orange-900/20">
          <label className="flex items-center gap-3 cursor-pointer">
            ...
            <div>
              ...
              <div className="text-sm text-orange-700 dark:text-orange-300">
                {t('permissions.antigravity.skipPermissionsDesc', {
                  defaultValue: '等同于 Antigravity CLI 的 --dangerously-skip-permissions 标志，完全自动执行所有命令与文件修改，无需弹窗确认。',
                })}
              </div>
            </div>
          </label>
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-lg font-medium text-foreground">
              {t('permissions.antigravity.permissionMode', { defaultValue: '执行权限模式 (Execution Mode)' })}
            </h3>
            <p className="text-sm text-muted-foreground">
              {t('permissions.antigravity.description', { defaultValue: '控制 Google Antigravity CLI 处理文件修改、工具调用和命令执行的确认策略' })}
            </p>
          </div>
        </div>
              ...
              <div
                ...
                className={`cursor-pointer rounded-lg border p-4 transition-all ${
                  isSelected
                    ...
                    : 'border-border bg-card/50 hover:bg-accent/40 active:bg-accent/60'
                }`}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      <span className="font-medium text-foreground">
                        {t(mode.titleKey, {
                          defaultValue:
                            mode.id === 'default' ? '默认常规模式 (Default)' :
                            mode.id === 'acceptEdits' ? '自动接受编辑 (Accept Edits)' :
                            mode.id === 'plan' ? '规划模式 (Plan Mode)' :
                            mode.id === 'sandbox' ? '沙箱模式 (Sandbox)' :
                            '绕过所有权限 (Bypass Permissions)',
                        })}
                      </span>
                    ...
                    <div className="mt-1 text-sm text-muted-foreground">
                      {t(mode.descKey, {
                        defaultValue:
                          mode.id === 'default' ? '交互询问确认。在执行修改文件或系统命令时遵循常规提示策略。' :
                          mode.id === 'acceptEdits' ? '代码与文件修改自动应用生效，减少反复弹窗确认，适合流畅编程。' :
                          mode.id === 'plan' ? '只读规划与推演模式，仅生成分析方案，不直接修改本地任何文件。' :
                          mode.id === 'sandbox' ? '在隔离沙箱环境中运行，增强系统安全性。' :
                          '完全自动执行所有命令与修改，无任何确认提示，请在受信任的项目环境中使用。',
                      })}
                    </div>
                  </div>
                </label>
        ...
        <details className="text-sm">
          <summary className="cursor-pointer text-muted-foreground hover:text-foreground">
            {t('permissions.antigravity.technicalDetails', { defaultValue: 'Antigravity CLI 命令行参数技术详情' })}
          </summary>
          <div className="mt-2 space-y-2 rounded-lg bg-muted/50 p-3 text-xs text-muted-foreground">
            <p><strong>Default:</strong> CLI 标准启动参数，遵循默认交互询问策略。</p>
            <p><strong>Accept Edits:</strong> 附加 <code>--mode accept-edits</code> 参数，自动批准代码文件读写。</p>
            <p><strong>Plan Mode:</strong> 附加 <code>--mode plan</code> 参数，仅提供方案分析，不写入代码。</p>
            <p><strong>Sandbox:</strong> 附加 <code>--sandbox</code> 参数，将进程运行约束在沙箱中。</p>
            <p><strong>Bypass Permissions:</strong> 附加 <code>--dangerously-skip-permissions</code> 参数，跳过所有权限检查。</p>
            <p className="text-xs opacity-75">{t('permissions.antigravity.overrideNote', { defaultValue: '您也可以在聊天会话界面的底部模式选择按钮中针对单次对话临时切换此模式。' })}</p>
          </div>
        </details>
      </div>
    </div>

          ...
          <div className="flex min-w-0 flex-1 items-center gap-3">
            ...
            <div className="min-w-0 flex-1">
              <button
                ...
                title={t('tasks:detail.copyId', '复制任务 ID')}
              >
                <span>{t('tasks:detail.taskPrefix', '任务')} {task.id}</span>
            ...
            {isEditMode ? (
              <>
                ...
                <button
                  ...
                  title={t('common:actions.cancel', '取消编辑')}
                >
                  <X className="h-5 w-5" />
                </button>
              </>
              ...
              <button
                ...
                className="rounded-md p-2 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800"
                title={t('tasks:detail.editTask', '编辑任务')}
              >
                <Edit className="h-5 w-5" />
              </button>
            )}
            <button onClick={onClose} className="rounded-md p-2 text-gray-500 hover:bg-gray-100 dark:hover:bg-gray-800" title={t('common:actions.close', '关闭')}>
              <X className="h-5 w-5" />
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 space-y-6 overflow-y-auto p-4 md:p-6">
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('tasks:columns.status', '状态')}</label>
              <select
              ...
              >
                {STATUS_OPTIONS.map((option) => (
                  <option key={option.value} value={option.value}>
                    {t('tasks:status.' + option.value, option.label)}
                  </option>
                ))}
              </select>
            </div>
            ...
            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('tasks:detail.priority', '优先级')}</label>
              <div className={cn('px-3 py-2 rounded-md text-sm font-medium capitalize', getPriorityBadgeClass(task.priority))}>
                {task.priority ? t('tasks:priority.' + task.priority, task.priority) : t('tasks:detail.notSet', '未设置')}
              </div>
            </div>

            <div className="space-y-2">
              <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('tasks:detail.dependencies', '依赖任务')}</label>
              {Array.isArray(task.dependencies) && task.dependencies.length > 0 ? (
                ...
                <span className="text-sm text-gray-500 dark:text-gray-400">{t('tasks:detail.noDependencies', '无依赖任务')}</span>
              )}
            </div>
          </div>

          <div className="space-y-2">
            <label className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('tasks:detail.description', '任务描述')}</label>
            {isEditMode ? (
              <textarea
                rows={4}
              ...
              <p className="whitespace-pre-wrap text-gray-700 dark:text-gray-300">{task.description || t('tasks:detail.noDescription', '未提供任务描述')}</p>
            )}
          </div>
          ...
          {task.details && (
            <div className="rounded-lg border border-gray-200 dark:border-gray-700">
              ...
              >
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('tasks:detail.implementationDetails', '实现详情')}</span>
          ...
          {task.testStrategy && (
            <div className="rounded-lg border border-gray-200 dark:border-gray-700">
              <button
                ...
                className="flex w-full items-center justify-between p-4 text-left hover:bg-gray-50 dark:hover:bg-gray-800"
              >
                <span className="text-sm font-medium text-gray-700 dark:text-gray-300">{t('tasks:detail.testStrategy', '测试策略')}</span>

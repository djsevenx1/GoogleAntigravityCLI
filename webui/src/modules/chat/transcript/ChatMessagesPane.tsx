...
function checkIsEndOfTask(
  items: (ChatMessage | ToolGroupItem)[],
  index: number,
  isProcessing: boolean
): boolean {
  const current = items[index];
  if (isToolGroupItem(current)) return false;
  if (current.type !== 'assistant' || current.isThinking || current.isToolUse) return false;
  if (!(current.content || '').trim()) return false;

  // Look ahead to see if another assistant message or tool (single or group) follows in the same task/turn
  for (let i = index + 1; i < items.length; i++) {
    const next = items[i];
    if (isToolGroupItem(next) || next.isToolUse || next.isThinking) {
      // Tool call, tool group, or thinking follows in this turn
      return false;
    }
    if (next.type === 'user') {
      // Reached the next user turn without intermediate tool calls or replies
      return true;
    }
    if (next.type === 'assistant' && (next.content || '').trim()) {
      // Another assistant response follows in this task
      return false;
    }
  }

  // At the tail of the message list: it is the end of the task only if not processing and not streaming
  return !isProcessing && !current.isStreaming;
}
        ...
        <>
          ...
          {(() => {
            ...
            return groupedVisibleMessages.map((item, index) => {
              ...
              const isEndOfTask = checkIsEndOfTask(groupedVisibleMessages, index, Boolean(isProcessing));

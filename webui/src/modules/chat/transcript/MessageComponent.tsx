 ...
 */
const MessageComponent = memo(({ message, prevMessage, createDiff, onFileOpen, showRawParameters, showThinking, selectedProject, provider, isEndOfTask, onEditMessage, onForkFromMessage }: MessageComponentProps) => {
  ...
  const isToolLikeError = useMemo(() => {
    if (message.type !== 'error') return false;
    const s = String(message.content || '');
    return (
      /不存在|does not exist|no such (file|directory)|timed out|超时|context deadline exceeded|cannot kill task|Encountered error in (step|tool) execution|stat .* no such|failed to read file|invalid tool call|declaring permissions|exit code \d+/i.test(s)
    );
      ...
      {message.type === 'user' ? (
        ...
        <div className="w-full">
          {!isGrouped && (
            <div className="mb-2 flex items-center space-x-3">
              ...
              <div className="text-sm font-medium text-gray-900 dark:text-white">
                {message.type === 'error'
                  ? (isToolLikeError ? '工具执行反馈' : (provider === 'antigravity' ? 'Antigravity 错误' : t('messageTypes.error')))
                  : message.type === 'tool'
                    ? t('messageTypes.tool')
                    : (provider === 'cursor'
                        ...
                        : provider === 'codex'
                          ...
                          : provider === 'opencode'
                              ...
                              : provider === 'antigravity'
                                  ? t('messageTypes.antigravity', { defaultValue: 'Google Antigravity' })
          ...
          <div className="w-full">
            ...
            {message.isSubagentContainer ? (
              ...
              <div dir="auto" className="text-sm text-gray-700 dark:text-gray-300">
                ...
                {(() => {
                  ...
                  const trimmedContent = (typeof content === 'string' ? content : String(content)).trim();
            ...
            {isEndOfTask && message.type === 'assistant' && !message.isStreaming && (
              <MessageQuotaFooter message={message} provider={provider} />
            )}

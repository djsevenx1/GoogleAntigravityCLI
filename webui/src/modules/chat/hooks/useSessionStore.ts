...
export function useSessionStore() {
   ...
   */
  const finalizeStreaming = useCallback((sessionId: string, extraProps?: Record<string, unknown>) => {
    const slot = storeRef.current.get(sessionId);
    if (!slot) return;
    const streamId = `__streaming_${sessionId}`;
    const idx = slot.realtimeMessages.findIndex(m => m.id === streamId);
    if (idx >= 0) {
      ...
      recomputeMergedIfNeeded(slot);
      notify(sessionId);
    } else if (extraProps && Object.keys(extraProps).length > 0) {
      // If streaming was already finalized (e.g. by stream_end), update the last assistant message
      // with the turn metadata (quotaSnapshot, meta, tokens, duration, model).
      let updated = false;
      for (let i = slot.realtimeMessages.length - 1; i >= 0; i--) {
        ...
        if (m.role === 'assistant' || m.kind === 'text') {
          ...
          slot.realtimeMessages[i] = {
            ...
            meta: {
              ...((m as any).meta || {}),
              ...((extraProps.meta as any) || {}),
            },
          } as any as NormalizedMessage;
          updated = true;
          break;
        }
      }
      if (!updated && slot.serverMessages.length > 0) {
        for (let i = slot.serverMessages.length - 1; i >= 0; i--) {
          const m = slot.serverMessages[i];
          if (m.role === 'assistant' || m.kind === 'text') {
            slot.serverMessages = [...slot.serverMessages];
            slot.serverMessages[i] = {
              ...m,
              ...(extraProps || {}),
              meta: {
                ...((m as any).meta || {}),

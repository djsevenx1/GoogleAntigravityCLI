...
export function useChatComposerState({
  ...
  const input = typeof inputState?.value === 'string' ? inputState.value : '';
  ...
  const setInput = useCallback<Dispatch<SetStateAction<string>>>((next) => {
    setInputState((previous) => {
      const prevVal = typeof previous?.value === 'string' ? previous.value : '';
      const resolved = typeof next === 'function' ? next(prevVal) : next;
      return {
      ...
      if (commandInput.startsWith('/') || isHelpAlias) {
        ...
        const matchedCommand =
          ...
          (commandName === '/help'
            ? ({
                name: '/help',
                description: 'Show help documentation for Antigravity cli',
                namespace: 'builtin',
                metadata: { type: 'builtin' },
              } as SlashCommand)
            : undefined);
  ...
  useEffect(() => {
    ...
    const restoreDraft = () => {
      ...
      const safeInput = typeof savedInput === 'string' ? savedInput : '';
      setInputState((previous) => {
        if (previous?.scope === draftScope && previous?.value === safeInput) {
          return previous;
        }
        inputValueRef.current = safeInput;
        return { scope: draftScope, value: safeInput };
      });
    };
  ...
  useEffect(() => {
    ...
    if (
      queuedDraft
      && ((typeof queuedDraft.content === 'string' && queuedDraft.content.trim()) || (queuedDraft.uploadedAttachments?.length ?? 0) > 0)
    ) {

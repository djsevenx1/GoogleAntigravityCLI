...
export function normalizedToChatMessages(messages: NormalizedMessage[]): ChatMessage[] {
  ...
  for (const msg of messages) {
    ...
    const sharedMetadata = {
      ...
      quotaSnapshot: (msg as any).quotaSnapshot,
      meta: (msg as any).meta,
      model: (msg as any).model || (msg as any).meta?.model,
      duration: (msg as any).duration || (msg as any).meta?.duration,
      tokens: (msg as any).tokens || (msg as any).meta?.tokens,
    };
    ...
    switch (msg.kind) {
      ...
      case 'tool_use': {
        ...
        const toolResult = tr
          ? {
              // Antigravity's tool_result rows carry their text under
              // `toolResult.content` with no top-level `content`, so fall back
              // to the nested field or the crash guard in formatToolResultContent
              // would silently render every Antigravity tool output as blank.
              content: formatToolResultContent(tr.content ?? (tr as any).toolResult?.content),
        ...
        converted.push({
          ...
          content: typeof msg.content === 'string'
            ...
            : (typeof msg.text === 'string'
                ? msg.text
                : (msg.content ? JSON.stringify(msg.content) : 'Unknown error')),
          timestamp: msg.timestamp,
          ...sharedMetadata,
        });
        break;
      ...
      case 'tool_result': {
        ...
        const content = formatToolResultContent(msg.content ?? (msg as any).toolResult?.content ?? '');
        if (!content.trim()) {
          break;
        }

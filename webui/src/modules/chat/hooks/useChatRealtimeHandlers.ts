...
export function useChatRealtimeHandlers({
  ...
  useEffect(() => {
    const handleEvent = (msg: ServerEvent) => {
      ...
      const shouldPersist =
        msg.kind !== 'complete'
        && (msg.kind as string) !== 'quota_update'
        && msg.kind !== 'status'
      ...
      switch (msg.kind as string) {
        case 'quota_update': {
          const snap = (msg as any).quotaSnapshot;
          if (snap) {
            updateCachedAntigravityQuota(snap);
          }
          break;
        }
        case 'complete': {
          ...
          const extraProps = {
            ...
            model: (msg as any).meta?.model || (msg as any).model,
            duration: (msg as any).meta?.duration,
            tokens: (msg as any).meta?.tokens,
          };
          if (sid) {
            sessionStore.finalizeStreaming(sid, extraProps);
          }
          accumulatedStreamRef.current = '';

          // Proactively update and fetch latest Google Antigravity quota upon turn completion
          const isAgy = provider === 'antigravity' || (msg as any).provider === 'antigravity';
          const snap = (msg as any).quotaSnapshot;
          if (snap) {
            updateCachedAntigravityQuota(snap);
          }
          if (isAgy) {
            void fetchAntigravityQuota(true, snap?.accountEmail);
          }

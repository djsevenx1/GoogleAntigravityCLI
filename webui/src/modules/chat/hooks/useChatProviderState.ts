...
const PROVIDERS: LLMProvider[] = ['antigravity'];
...
export function useChatProviderState({ selectedSession, selectedProject: _selectedProject }: UseChatProviderStateArgs) {
  ...
  useEffect(() => {
    ...
    const loadSessionSelection = async () => {
      try {
        ...
        const rawModel = body.data?.model;
        const rawEffort = body.data?.effort;
        const resolvedModel = typeof rawModel === 'string' ? rawModel.trim() : (rawModel ? String(rawModel).trim() : null);
        const resolvedEffort = typeof rawEffort === 'string' ? rawEffort.trim() : (rawEffort ? String(rawEffort).trim() : null);
        setSessionSelection({
  ...
  const selectProviderModel = useCallback(async (
    ...
    const rawReturnedModel = body.data?.model;
    const storedModel = typeof rawReturnedModel === 'string' ? rawReturnedModel.trim() : model;
    if (
      sessionModelMutationIdRef.current === mutationId
      && selectedSessionKeyRef.current === targetSessionKey
    ) {
      const rawReturnedEffort = body.data?.effort;
      const parsedReturnedEffort = typeof rawReturnedEffort === 'string' ? rawReturnedEffort.trim() : null;
  ...
  const selectProviderEffort = useCallback(async (
    ...
    try {
      ...
      const rawStoredEffort = body.data?.effort;
      const storedEffort = typeof rawStoredEffort === 'string' ? rawStoredEffort.trim() : effort;
      if (
        sessionEffortMutationIdRef.current === mutationId
        && selectedSessionKeyRef.current === targetSessionKey
      ) {

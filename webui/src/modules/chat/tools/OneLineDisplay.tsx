...
export const OneLineDisplay: React.FC<OneLineDisplayProps> = ({
  ...
  // Terminal style: clean card (white in light mode, dark in dark mode)
  if (isTerminal) {
    return (
      <div className="group my-1">
        <div className="flex items-start gap-2">
          ...
          <div className="flex min-w-0 flex-1 items-start gap-2">
            <div className="min-w-0 flex-1 rounded-md border border-zinc-200/90 bg-white px-2.5 py-1.5 shadow-xs dark:border-zinc-800 dark:bg-zinc-900/90">
              <code className={`font-mono text-xs text-zinc-800 dark:text-zinc-200 ${wrapText ? 'whitespace-pre-wrap break-all' : 'block truncate'}`}>
                <span className="select-none font-semibold text-emerald-600 dark:text-emerald-400">$ </span>{value}
              </code>
            </div>
            {status && <ToolStatusBadge status={status} className="mt-1" />}
            {action === 'copy' && <div className="mt-1">{renderCopyButton()}</div>}
          </div>
        </div>
        {secondary && (

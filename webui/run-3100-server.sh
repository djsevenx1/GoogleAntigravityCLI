...
LOCKFILE="/tmp/agy-server-3100.lock"
exec 200>"$LOCKFILE"
flock -n 200 || {
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] 已有 3100 实例在运行中，直接退出"
  exit 0
}
...
export SERVER_PORT="${SERVER_PORT:-3100}"
export PORT="${PORT:-3100}"
export HOST="${HOST:-::}"
export VITE_PORT="${VITE_PORT:-5178}"
export HOME="${AGY_HOME:-$BASE_DIR/home}"
export DATABASE_PATH="${DATABASE_PATH:-$BASE_DIR/home/.cloudcli/auth.db}"
export AGY_BIN="${AGY_BIN:-$BASE_DIR/bin/antigravity}"
export AGY_HOME="${AGY_HOME:-$BASE_DIR/home}"
export WORKSPACES_ROOT="${WORKSPACES_ROOT:-$BASE_DIR}"

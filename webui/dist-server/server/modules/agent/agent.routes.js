 ...
 */
export function createAgentRouter(dependencies) {
    ...
    async function cloneGitHubRepo(githubUrl, githubToken = null, projectPath) {
        return new Promise(async (resolve, reject) => {
            try {
                ...
                if (parsedGithubUrl.protocol !== 'https:'
    ...
    async function cleanupProject(projectPath, sessionId = null) {
        try {
            const externalProjectsRoot = await fs.realpath(path.join(os.homedir(), '.claude', 'external-projects'));
    ...
    class ResponseCollector {
        ...
        getAssistantMessages() {
            ...
            for (const msg of this.messages) {
                ...
                // Handle JSON strings (Claude SDK style events)
                if (typeof msg === 'string') {
                    ...
                    // Normalized provider messages (stream deltas, thinking, tool use,
                    // tool results). Keep assistant-produced content and drop terminal
                    // bookkeeping like stream_started / stream_complete events.
                    if (msg.kind === 'stream_delta' || msg.kind === 'text') {
                        ...
                        assistantMessages.push({
                            type: 'thinking',
                            thinking: msg.content ?? msg.thinking ?? '',
                        });
                    }
        ...
        getTotalTokens() {
            ...
            for (const msg of this.messages) {
                ...
                else if (data && typeof data === 'object' && (data.usage || data.tokenUsage)) {
                    // Normalized provider messages carrying a usage block (e.g. the
                    // Antigravity CLI result event).
                    const usage = data.usage || data.tokenUsage;
                    totalInput += usage.input_tokens || usage.inputTokens || 0;
                    totalOutput += usage.output_tokens || usage.outputTokens || 0;
                    totalCacheRead += usage.cache_read_tokens || usage.cacheReadTokens || 0;
                    totalCacheCreation += usage.cache_creation_tokens || usage.cacheCreationTokens || 0;
                }
            }
    ...
    router.post('/', validateExternalApiKey, async (req, res) => {
        const { githubUrl, projectPath, message, provider = 'antigravity', model, githubToken, branchName, sessionId } = req.body;
        ...
        if (provider !== 'antigravity') {
            return res.status(400).json({ error: 'provider must be "antigravity"' });
        }
        ...
        try {
            ...
            if (provider === 'claude') {
                ...
                console.log('Starting Google Antigravity CLI session');

...
import { findWorktreeEntryByPath, parseWorktreeListPorcelain, validateWorktreeBranchName, } from '../../../modules/worktrees/services/worktree-git.service.js';
...
test('parseWorktreeListPorcelain handles output without a trailing blank line', () => {
    const entries = parseWorktreeListPorcelain('worktree /home/user/repo\nHEAD 1111111111111111111111111111111111111111\nbranch refs/heads/main');
...
test('findWorktreeEntryByPath throws a 404 AppError for unknown paths', () => {
    const entries = parseWorktreeListPorcelain(SAMPLE_PORCELAIN);
    assert.throws(() => findWorktreeEntryByPath(entries, '/home/user/elsewhere'), (error) => error instanceof AppError && error.code === 'WORKTREE_NOT_FOUND' && error.statusCode === 404);
});
...
test('validateWorktreeBranchName rejects unsafe names', () => {
    for (const invalidName of [
        ...
        assert.throws(() => validateWorktreeBranchName(invalidName), (error) => error instanceof AppError && error.code === 'INVALID_BRANCH_NAME', `expected "${invalidName}" to be rejected`);
    }
});
//# sourceMappingURL=worktree-git.service.test.js.map
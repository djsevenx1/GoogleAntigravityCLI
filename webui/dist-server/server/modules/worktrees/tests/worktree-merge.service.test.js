...
function createDependencies(runner) {
    return {
        runGit: runner,
...
test('mergeWorktree rejects when the source worktree is dirty', async () => {
    ...
    await assert.rejects(mergeWorktree({
        projectPath: '/home/user/repo',
        worktreePath: '/home/user/repo-worktrees/feature-login',
    }, createDependencies(runner)), (error) => error instanceof AppError && error.code === 'WORKTREE_SOURCE_DIRTY' && error.statusCode === 409);
});
test('mergeWorktree aborts and reports conflicted files on merge conflict', async () => {
    ...
    await assert.rejects(mergeWorktree({
        projectPath: '/home/user/repo',
        worktreePath: '/home/user/repo-worktrees/feature-login',
    }, createDependencies(runner)), (error) => error instanceof AppError &&
...
test('mergeWorktree refuses to merge the main worktree into itself', async () => {
    ...
    }, createDependencies(runner)), (error) => error instanceof AppError && error.code === 'WORKTREE_MERGE_MAIN');
});
//# sourceMappingURL=worktree-merge.service.test.js.map
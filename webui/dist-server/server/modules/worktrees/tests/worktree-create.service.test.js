 ...
 */
function createFakeRunner(existingBranches) {
    const calls = [];
...
function createDependencies(runner, pathExists = false) {
    return {
        runGit: runner,
        fileSystem: {
...
test('createWorktree creates a new branch from the main branch by default', async () => {
    ...
    const result = await createWorktree({ projectPath: '/home/user/repo', branch: 'feature/login' }, createDependencies(runner));
...
test('createWorktree checks out an existing branch without -b', async () => {
    ...
    const result = await createWorktree({ projectPath: '/home/user/repo', branch: 'bugfix' }, createDependencies(runner));
...
test('createWorktree honors an explicit base branch', async () => {
    const { calls, runner } = createFakeRunner([]);
    await createWorktree({ projectPath: '/home/user/repo', branch: 'hotfix', baseBranch: 'release/1.0' }, createDependencies(runner));
    const addCall = calls.find((call) => call.args[0] === 'worktree' && call.args[1] === 'add');
    assert.ok(addCall);
...
test('createWorktree rejects a branch already checked out in another worktree', async () => {
    ...
    await assert.rejects(createWorktree({ projectPath: '/home/user/repo', branch: 'existing-branch' }, createDependencies(runner)), (error) => error instanceof AppError && error.code === 'BRANCH_ALREADY_CHECKED_OUT' && error.statusCode === 409);
});
test('createWorktree rejects invalid branch names before touching git state', async () => {
    const { calls, runner } = createFakeRunner([]);
    await assert.rejects(createWorktree({ projectPath: '/home/user/repo', branch: '-rf' }, createDependencies(runner)), (error) => error instanceof AppError && error.code === 'INVALID_BRANCH_NAME');
    assert.equal(calls.length, 0);
});
test('createWorktree rejects an occupied destination without running a mutating git command', async () => {
    const { calls, runner } = createFakeRunner([]);
    await assert.rejects(createWorktree({ projectPath: '/home/user/repo', branch: 'feature/login' }, createDependencies(runner, true)), (error) => error instanceof AppError && error.code === 'WORKTREE_FOLDER_EXISTS');
    assert.equal(calls.some((call) => call.args[0] === 'worktree' && call.args[1] === 'add'), false);
});
//# sourceMappingURL=worktree-create.service.test.js.map
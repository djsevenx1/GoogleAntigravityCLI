...
test('listWorktrees bounds concurrent per-worktree Git inspection and preserves ordering', async () => {
    ...
    const result = await listWorktrees({ projectPath: '/workspace/repo-0' }, { runGit, getProjectByPath: () => null });
    assert.equal(maximumActiveCommands <= 12, true);
    assert.deepEqual(result.worktrees.map((worktree) => worktree.path.replace(/\\/g, '/')), Array.from({ length: entryCount }, (_, index) => `/workspace/repo-${index}`));
});
//# sourceMappingURL=worktree-list.service.test.js.map
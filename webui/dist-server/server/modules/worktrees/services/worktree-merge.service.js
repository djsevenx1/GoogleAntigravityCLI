...
import { countChangedFiles, findWorktreeEntryByPath, listWorktreePorcelainEntries, } from '../../../modules/worktrees/services/worktree-git.service.js';
/** Reads the paths currently in conflict inside the main worktree (best effort). */
async function readConflictedFiles(repositoryRoot, runGit) {
    try {
 ...
 */
export async function mergeWorktree(input, dependencies) {
    ...
    if (sourceChangedCount > 0) {
        throw new AppError(`Worktree has ${sourceChangedCount} uncommitted change${sourceChangedCount === 1 ? '' : 's'} — commit or discard them first`, { code: 'WORKTREE_SOURCE_DIRTY', statusCode: 409 });
    }
    const targetChangedCount = await countChangedFiles(repositoryRoot, runGit);
    if (targetChangedCount > 0) {
        throw new AppError(`The base worktree (${targetBranch}) has uncommitted changes — commit or stash them first`, { code: 'WORKTREE_TARGET_DIRTY', statusCode: 409 });
    }
    ...
    try {
        ...
        try {
            ...
            throw new AppError(`Merge of '${entry.branch}' into '${targetBranch}' failed and could not be rolled back`, {
        ...
        if (conflictedFiles.length > 0) {
            throw new AppError(`Merge of '${entry.branch}' into '${targetBranch}' has conflicts — the merge was aborted`, {

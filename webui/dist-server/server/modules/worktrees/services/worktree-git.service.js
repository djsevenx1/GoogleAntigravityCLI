...
export function runGitCommand(args, cwd) {
    return new Promise((resolve, reject) => {
        ...
        child.on('error', (error) => {
            reject(new AppError(`Failed to run git: ${error.message}`, {
        ...
        child.on('close', (code) => {
            ...
            reject(new AppError(`git ${args.join(' ')} failed`, {
 ...
 */
export function validateWorktreeBranchName(branch) {
    ...
    if (!trimmed ||
        ...
        trimmed.includes('..') ||
        trimmed.includes('//') ||
        components.some((component) => component.startsWith('.') || component.toLowerCase().endsWith('.lock')) ||
 ...
 */
export async function listWorktreePorcelainEntries(projectPath, runGit) {
    let output;
    try {
 ...
 */
export function findWorktreeEntryByPath(entries, worktreePath) {
 ...
 */
export async function countChangedFiles(worktreePath, runGit) {
    const { stdout } = await runGit(['status', '--porcelain'], worktreePath);
    return stdout.split('\n').filter((line) => line.trim().length > 0).length;
}
//# sourceMappingURL=worktree-git.service.js.map
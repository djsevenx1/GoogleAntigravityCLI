...
import { listWorktreePorcelainEntries, validateWorktreeBranchName, } from '../../../modules/worktrees/services/worktree-git.service.js';
/**
 * Turns a branch name into a filesystem-safe folder name:
 * "feature/login-form" → "feature-login-form".
 */
function sanitizeBranchForDirectoryName(branch) {
 ...
 */
export async function createWorktree(input, dependencies) {
    ...
    const worktreesContainer = path.join(path.dirname(repositoryRoot), `${path.basename(repositoryRoot)}-worktrees`);
    const worktreePath = normalizeProjectPath(path.join(worktreesContainer, sanitizeBranchForDirectoryName(branch)));
    ...
    const { stdout: branchListOutput } = await runGit(['branch', '--list', branch, '--format=%(refname:short)'], repositoryRoot);
    ...
    if (branchExists) {
        ...
        await runGit(['worktree', 'add', worktreePath, '-b', branch, validateWorktreeBranchName(baseBranch)], repositoryRoot);
    }
    return { worktreePath, branch, createdBranch: !branchExists };
}
//# sourceMappingURL=worktree-create.service.js.map
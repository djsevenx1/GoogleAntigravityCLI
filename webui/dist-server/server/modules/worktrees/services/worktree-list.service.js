...
import { countChangedFiles, listWorktreePorcelainEntries, } from '../../../modules/worktrees/services/worktree-git.service.js';
 ...
 */
async function countAheadBehind(repositoryRoot, baseBranch, branch, runGit) {
    if (!baseBranch || !branch || baseBranch === branch) {
        return { ahead: 0, behind: 0 };
    }
    try {
        const { stdout } = await runGit(['rev-list', '--left-right', '--count', `${baseBranch}...${branch}`], repositoryRoot);
...
async function readLastCommit(worktreePath, runGit) {
    try {
 ...
 */
export async function listWorktrees(input, dependencies) {
    ...
    const worktrees = await mapWithConcurrency(entries, WORKTREE_LIST_CONCURRENCY, async (entry, index) => {
...
//# sourceMappingURL=worktree-list.service.js.map
...
import { filterAttachmentsToUploadStore, filterImagesToUploadStore, } from '../../../modules/websocket/services/chat-websocket.service.js';
...
test('images inside the upload store pass through', () => {
    ...
    const result = filterImagesToUploadStore([{ path: inside, name: 'shot.png', mimeType: 'image/png' }], STORE);
...
test('general files inside the upload store preserve their metadata', () => {
    ...
    const result = filterAttachmentsToUploadStore([{ path: inside, name: 'brief.pdf', mimeType: 'application/pdf', size: 2048 }], STORE);
    assert.deepEqual(result, [
        { path: inside, name: 'brief.pdf', mimeType: 'application/pdf', size: 2048 },
    ]);
});
//# sourceMappingURL=chat-attachment-filter.test.js.map
...
import { broadcastSessionUpserted, broadcastSessionUpsertedBatch, } from '../../../modules/websocket/services/session-upsert-broadcast.service.js';
...
test('a batch delivers every resolvable session and skips the rest', async () => {
    await withIsolatedDatabase(async () => {
        ...
        assert.deepEqual(connection.frames.map((frame) => frame.sessionId), ['app-4', 'app-5']);
    });
});

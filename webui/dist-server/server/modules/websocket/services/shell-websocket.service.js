...
function shouldAutoOpenUrlFromOutput(value) {
    ...
    return (normalizedOutput.includes("browser didn't open") ||
 ...
 */
function parseShellMessage(rawMessage) {
...
function resolveResumeSessionId(message, dependencies) {
 ...
 */
function buildShellCommand(message, dependencies) {
    const hasSession = readBoolean(message.hasSession);
    const initialCommand = readString(message.initialCommand);
    const provider = readString(message.provider, 'antigravity');
    const resumeSessionId = resolveResumeSessionId(message, dependencies);
    const isPlainShell = readBoolean(message.isPlainShell) ||
    ...
    if (provider === 'antigravity') {
        return initialCommand || 'antigravity';
    }
...
function prioritizeUserNpmGlobalBin(env) {
    ...
    const normalizedPreferredEntries = preferredEntries.map((entry) => os.platform() === 'win32' ? entry.toLowerCase() : entry);
    const value = [
        ...preferredEntries,
 ...
 */
export function handleShellConnection(ws, dependencies) {
    ...
    ws.on('message', async (rawMessage) => {
        try {
            ...
            if (data.type === 'init') {
                ...
                const provider = readString(data.provider, 'antigravity');
                const initialCommand = readString(data.initialCommand);
                const forceRestart = readBoolean(data.forceRestart);
                const isPlainShell = readBoolean(data.isPlainShell) ||
                    (!!initialCommand && !hasSession) ||
                    provider === 'plain-shell';
                urlDetectionBuffer = '';
                ...
                const existingSession = isLoginCommand || forceRestart ? null : ptySessionsMap.get(ptySessionKey);
                if (existingSession) {
    ...
    ws.on('close', () => {
        ...
        session.timeoutId = setTimeout(() => {
            ...
            if (ptySessionsMap.get(ptySessionKey) !== session || session.ws !== null) {
                return;
            }
            session.pty.kill();
            ptySessionsMap.delete(ptySessionKey);
...
//# sourceMappingURL=shell-websocket.service.js.map
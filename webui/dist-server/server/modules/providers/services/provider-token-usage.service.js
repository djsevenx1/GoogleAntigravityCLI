...
const defaultDependencies = {
    ...
    isProviderSessionSuperseded: (providerSessionId, provider) => sessionsDb.isProviderSessionSuperseded(providerSessionId, provider),
};
function readUsageNumber(value) {
    const parsedValue = Number(value);
    return Number.isFinite(parsedValue) ? parsedValue : 0;
}
async function findCodexSessionFile(directoryPath, providerSessionId, dependencies) {
    let entries;
    try {
...
function findCodexTokenUsage(fileContent) {
 ...
 * once per turn.
 */
export function summarizeClaudeTokenUsage(entries, configuredContextWindow = process.env.CONTEXT_WINDOW) {
    let inputTokens = 0;
    let outputTokens = 0;
    ...
    for (let index = entries.length - 1; index >= 0; index -= 1) {
        ...
        const rowCacheCreationTokens = readUsageNumber(usage.cache_creation_input_tokens
...
function parseClaudeUsageEntries(fileContent) {
    const entries = [];
...
function claudeEntriesHaveUsage(entries) {
    return entries.some((entry) => entry?.type === 'assistant' && entry.message?.usage);
}
function readOpenCodeTokenUsage(databasePath, providerSessionId) {
 ...
 */
export function createProviderTokenUsageService(dependencyOverrides = {}) {
        ...
        async getSessionTokenUsage(sessionId) {
            ...
            if (session.provider === 'antigravity') {
                try {
                    ...
                    const quota = await antigravityAccountsService.getTurnQuotaSnapshot();
                    const usedPct = quota ? quota.active5h.used : 0;
                    const remainingPct = quota ? quota.active5h.percent : 100;
                    return {
                        used: usedPct,
                        total: 100,
                        inputTokens: 0,
            ...
            if (session.provider === 'codex') {
                const indexedFilePath = session.jsonl_path && dependencies.fileExists(session.jsonl_path)
                    ? session.jsonl_path
                    : null;
                const sessionFilePath = indexedFilePath ?? await findCodexSessionFile(path.join(dependencies.getHomeDirectory(), '.codex', 'sessions'), providerSessionId, dependencies);
                if (!sessionFilePath) {
            ...
            if (!sessionFilePath) {
                ...
                const projectDirectory = path.join(dependencies.getHomeDirectory(), '.claude', 'projects', encodedProjectPath);
...
//# sourceMappingURL=provider-token-usage.service.js.map
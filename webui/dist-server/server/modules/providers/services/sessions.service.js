...
function buildCloudCliSessionName(initialMessage) {
...
export const sessionsService = {
     ...
     */
    async forkSessionById(sessionId, options = {}) {
    ...
    async resolveEditAnchor(sessionId, anchorId) {
        ...
        const sessions = providerRegistry.resolveProvider(session.provider).sessions;
     ...
     */
    providerRewindsForEdit(sessionId) {
        const session = sessionsDb.getSessionById(sessionId);
        if (!session) {
            throw new AppError(`Session "${sessionId}" was not found.`, {
                code: 'SESSION_NOT_FOUND',
        ...
        return Boolean(providerRegistry.resolveProvider(session.provider).sessions.rewindSession);
    },
    /**
     ...
     */
    async rewindSessionForEdit(sessionId, keepThroughId) {
        const session = sessionsDb.getSessionById(sessionId);
        if (!session) {
            throw new AppError(`Session "${sessionId}" was not found.`, {
                code: 'SESSION_NOT_FOUND',
     ...
     */
    getSessionDetailsById(sessionId) {
    ...
    async deleteOrArchiveSessionById(sessionId, options = {}) {
        ...
        if (options.deletedFromDisk) {
            if (session.provider === 'antigravity') {
                try {
                    const { resolveAntigravityStateDir } = await import('../../../modules/providers/list/antigravity/antigravity-auth.provider.js');
                    const providerSessionId = session.provider_session_id || session.session_id;
                    const stateDir = resolveAntigravityStateDir();
                    const brainDir = path.join(stateDir, 'brain', providerSessionId);
                    await fsp.rm(brainDir, { recursive: true, force: true }).catch(() => { });
                    for (const ext of ['', '-shm', '-wal']) {
                        await fsp.rm(path.join(stateDir, 'conversations', `${providerSessionId}.db${ext}`), { force: true }).catch(() => { });
                    }
                    await fsp.rm(path.join(stateDir, 'presence', `${providerSessionId}.lock`), { force: true }).catch(() => { });
                    await fsp.rm(path.join(stateDir, 'annotations', `${providerSessionId}.pbtxt`), { force: true }).catch(() => { });

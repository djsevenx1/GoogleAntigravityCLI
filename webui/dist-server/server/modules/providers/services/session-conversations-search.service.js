 ...
 */
function findSessionTitleResults(sessions, query, limit) {
    ...
    return sessions
        ...
        if (!projectCache.has(projectKey)) {
            projectCache.set(projectKey, projectPath ? projectsDb.getProjectPath(projectPath) : null);
        }
...
function normalizeSearchableSessions(rows) {
...
function buildProjectBuckets(searchableSessions) {
...
async function parseClaudeSessionMatches(session, runtime) {
    ...
    if (!runtime.claudeFileResultsCache.has(fileKey)) {
        ...
        const providerToInternalId = new Map();
 ...
 */
export async function searchConversations(query, limit = 50, onProjectResult = null, signal = null, onTitleResults = null) {
...
export const sessionConversationsSearchService = {
     ...
     */
    async search(input) {
        await searchConversations(input.query, input.limit, input.onProgress ?? null, input.signal ?? null, input.onTitleResults ?? null);
    },
};

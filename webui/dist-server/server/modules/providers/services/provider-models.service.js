...
const toCustomProviderModelOption = (record) => ({
    value: record.modelId,
    label: record.model,
    recordId: record.recordId,
    isCustom: true,
});
const mergeProviderModels = (predefined, custom) => {
    return {
        OPTIONS: [
...
const normalizeCustomModelInput = (input) => ({
    id: input.id.trim(),
    model: input.model.trim(),
});
const isUniqueConstraintError = (error) => (error !== null
    && error !== undefined
    && typeof error === 'object'
    && 'code' in error
...
export const createProviderModelsService = (dependencies = {}) => {
    ...
    const readCustomModel = (provider, recordId) => {
    ...
    const assertModelIdAvailable = (provider, predefined, modelId, currentRecordId) => {
    ...
    const createCustomModel = async (provider, input) => {
        const predefined = await resolveProvider(provider).models.getSupportedModels();
    ...
    const updateCustomModel = async (provider, recordId, input) => {
        const predefined = await resolveProvider(provider).models.getSupportedModels();
    ...
    const deleteCustomModel = async (provider, recordId) => {
        const predefined = await resolveProvider(provider).models.getSupportedModels();
        readCustomModel(provider, recordId);
    ...
    const readRecordedSessionSelection = (sessionId) => {
     ...
     */
    const setSessionModel = (provider, sessionId, model) => {
     ...
     */
    const setSessionEffort = (provider, sessionId, effort) => {
        const normalizedSessionId = sessionId.trim();
     ...
     */
    const resolveSessionModel = async (provider, options = {}) => {

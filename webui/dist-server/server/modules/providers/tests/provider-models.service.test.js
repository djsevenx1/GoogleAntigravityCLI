...
const createModels = (value) => ({
    OPTIONS: [{ value, label: value }],
    DEFAULT: value,
});
const createCurrentActiveModel = (model) => ({ model });
/** In-memory stand-in for the `sessions` table rows the service reads and writes. */
const createSessionStore = (rows = {}, efforts = {}) => {
...
const createCatalogStore = () => {
    const rows = new Map();
    let nextRecordId = 1;
    const readRows = (provider) => rows.get(provider) ?? [];
    return {
        rows,
        ...
        updateCustomProviderModel(provider, recordId, input) {
            ...
            rows.set(provider, readRows(provider).map((record) => (record.recordId === recordId ? updated : record)));
            return updated;
        },
        deleteCustomProviderModel(provider, recordId, _fallbackModelId) {
            const existing = readRows(provider).find((record) => record.recordId === recordId);
            if (!existing) {
                return null;
            }
    ...
    const service = createProviderModelsService({
        ...
        resolveProvider: (provider) => ({
            models: {
                ...
                getCurrentActiveModel: async (sessionId) => createCurrentActiveModel(options.activeModel?.(provider, sessionId) ?? `${provider}-default`),
            },
        }),
    });
...
test('custom models can be created, edited, and deleted', async () => {
    ...
    const recordId = created.model.recordId;
...
test('duplicate model ids are rejected within one provider', async () => {
    ...
    await assert.rejects(() => service.createCustomModel('cursor', { model: 'Second', id: 'custom-id' }), (error) => error instanceof AppError
        && error.code === 'MODEL_ID_ALREADY_EXISTS'
        && error.statusCode === 409);
    await assert.rejects(() => service.createCustomModel('cursor', {
...
test('predefined models have no database record or mutation target', async () => {
    ...
    await assert.rejects(() => service.updateCustomModel('opencode', 999, { model: 'Changed', id: 'changed' }), (error) => error instanceof AppError && error.code === 'MODEL_NOT_FOUND');
});
...
//# sourceMappingURL=provider-models.service.test.js.map
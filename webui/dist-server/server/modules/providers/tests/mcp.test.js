...
test('providerMcpService handles claude MCP scopes/transports with file-backed persistence', { concurrency: false }, async () => {
    ...
    try {
        ...
        const projectServer = projectServers['claude-project-sse'];
...
test('providerMcpService handles codex MCP TOML config and capability validation', { concurrency: false }, async () => {
    ...
    try {
        ...
        const userConfig = TOML.parse(await fs.readFile(userTomlPath, 'utf8'));
        const userServers = userConfig.mcp_servers;
        const userStdio = userServers['codex-user-stdio'];
        assert.equal(userStdio.command, 'python');
        const projectTomlPath = path.join(workspacePath, '.codex', 'config.toml');
        const projectConfig = TOML.parse(await fs.readFile(projectTomlPath, 'utf8'));
        const projectServers = projectConfig.mcp_servers;
        const projectHttp = projectServers['codex-project-http'];
        assert.equal(projectHttp.url, 'https://codex.example.com/mcp');
        await assert.rejects(providerMcpService.upsertProviderMcpServer('codex', {
            name: 'codex-local',
            scope: 'local',
            transport: 'stdio',
            command: 'node',
...
test('providerMcpService handles opencode MCP config and capability validation', { concurrency: false }, async () => {
    ...
    await fs.writeFile(path.join(tempRoot, '.config', 'opencode', 'opencode.jsonc'), `{
    ...
    try {
        ...
        const userServers = userConfig.mcp;
        const userStdio = userServers['opencode-user-stdio'];
        ...
        const projectServers = projectConfig.mcp;
        const projectHttp = projectServers['opencode-project-http'];
        ...
        await assert.rejects(providerMcpService.upsertProviderMcpServer('opencode', {
            name: 'opencode-local',
            scope: 'local',
            transport: 'stdio',
            command: 'node',
...
test('providerMcpService handles cursor MCP JSON config formats', { concurrency: false }, async () => {
    ...
    try {
        ...
        const cursorHttpServer = cursorUserConfig.mcpServers['cursor-http'];
...
test('providerMcpService global adder writes to all providers and rejects unsupported transports', { concurrency: false }, async () => {
    ...
    try {
        ...
        assert.ok(claudeProject.mcpServers['global-http']);
        const antigravityProject = await readJson(path.join(workspacePath, '.antigravity', 'mcp.json'));
        assert.ok(antigravityProject.mcpServers['global-http']);
        const codexProject = TOML.parse(await fs.readFile(path.join(workspacePath, '.codex', 'config.toml'), 'utf8'));
        assert.ok(codexProject.mcp_servers['global-http']);
        const opencodeProject = await readJson(path.join(workspacePath, 'opencode.json'));
        assert.ok(opencodeProject.mcp['global-http']);
        const cursorProject = await readJson(path.join(workspacePath, '.cursor', 'mcp.json'));
        assert.ok(cursorProject.mcpServers['global-http']);
        await assert.rejects(providerMcpService.addMcpServerToAllProviders({
            name: 'global-sse',
            scope: 'project',
            transport: 'sse',
            url: 'https://example.com/sse',

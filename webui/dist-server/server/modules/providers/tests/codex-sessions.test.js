 ...
 */
const writeCodexTranscript = async (homeDir, codexSessionId, workspacePath, firstUserMessage) => {
...
test('Codex synchronizer skips sub-agent rollout files', { concurrency: false }, async () => {
    ...
    try {
        ...
        await mkdir(sessionsDir, { recursive: true });
        await writeFile(path.join(sessionsDir, 'rollout-codex-subagent-1.jsonl'), `${JSON.stringify({
            type: 'session_meta',
            payload: {
                id: 'codex-subagent-1',
                cwd: workspacePath,
    ...
    try {
        ...
        await withIsolatedDatabase(async () => {
            ...
            const byPath = new Map(edits.map((edit) => {
                const input = JSON.parse(String(edit.toolInput));
...
test('Codex history attaches a spawned agent\'s own transcript to the Task row', { concurrency: false }, async () => {
    ...
    try {
        ...
        // the agent thread id.
        await writeFile(path.join(path.dirname(transcriptPath), `rollout-2026-07-07T00-00-00-${agentThreadId}.jsonl`), `${[
            JSON.stringify({
                type: 'session_meta',
...
test('a plan followed by a memory citation is still recognized as a plan', async () => {
    ...
    try {
        ...
        await withIsolatedDatabase(async () => {
            ...
            assert.ok(!history.messages.some((message) => JSON.stringify(message).includes('oai-mem-citation')), 'no transcript row may still carry the raw citation markup');
        });
    }
    finally {
        restoreHomeDir();
...
test('Codex history restores user prompts from typed item_completed rows', { concurrency: false }, async () => {
    ...
    try {
        ...
        await writeFile(transcriptPath, [JSON.stringify({ type: 'session_meta', payload: { id: providerSessionId, cwd: workspacePath } }), typedLine, wireUserEcho].join(lineEnd) + lineEnd, 'utf8');
        await withIsolatedDatabase(async () => {
...
//# sourceMappingURL=codex-sessions.test.js.map
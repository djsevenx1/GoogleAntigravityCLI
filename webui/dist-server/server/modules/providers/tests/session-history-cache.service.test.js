...
async function withTranscriptFile(runTest) {
...
test('a missing transcript path or file bypasses the cache', async () => {
    ...
    assert.equal(await cache.getFullHistory({
        sessionId: 's1',
...
test('the oldest entries are evicted over budget, but the newest survives alone', async () => {
    ...
    try {
        ...
        const loadsBySession = new Map();
        const loaderFor = (sessionId) => async () => {
...
//# sourceMappingURL=session-history-cache.service.test.js.map
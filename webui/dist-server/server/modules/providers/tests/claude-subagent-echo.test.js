...
test('a subagent prompt echo is suppressed', () => {
    ...
    assert.equal(isSubagentPromptEcho({ kind: 'text', role: 'user', parentToolUseId: 'toolu_1', content: 'Investigate' }), true);
});
...
test('subagent tool traffic is kept', () => {
    assert.equal(isSubagentPromptEcho({ kind: 'tool_result', toolId: 'toolu_2', parentToolUseId: 'toolu_1' }), false);
});
//# sourceMappingURL=claude-subagent-echo.test.js.map
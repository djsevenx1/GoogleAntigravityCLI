...
import { findProviderSkillMarkdownFiles, readJsonConfig, readObjectRecord, readOptionalString, readProviderSkillMarkdownDefinition, } from '../../../../shared/utils.js';
const getClaudeHomePath = () => path.join(os.homedir(), '.claude');
const getClaudePluginName = (pluginId) => {
...
const stripMarkdownExtension = (filename) => filename.replace(/\.md$/i, '');
const pathExistsAsDirectory = async (directoryPath) => {
    try {
...
const readClaudePluginName = async (installPath, pluginId) => {
    try {
        const pluginConfig = await readJsonConfig(path.join(installPath, '.claude-plugin', 'plugin.json'));
...
export class ClaudeSkillsProvider extends SkillsProvider {
    ...
    async listPluginSkills(claudeHomePath) {
        ...
        const installedConfig = await readJsonConfig(path.join(claudeHomePath, 'plugins', 'installed_plugins.json'));
        const installedPlugins = readObjectRecord(installedConfig.plugins);
        if (!installedPlugins) {
            return [];
        }
        const skills = [];
        const visitedPluginFolders = new Set();
        ...
        for (const [pluginId, enabled] of pluginEntries) {
            ...
            for (const install of installs) {
                ...
                for (const pluginFolder of pluginFolders) {
                    ...
                    skills.push(...(await this.listPluginSkillMarkdowns(pluginFolder, pluginId, pluginName)));
                }
            }
        }
        return skills;
    }
    async listPluginCommandSkills(commandsPath, pluginId, pluginName) {
        const skills = [];
        try {
    ...
    async readPluginCommandDefinition(commandPath) {
    ...
    async listPluginSkillMarkdowns(installPath, pluginId, pluginName) {
...
//# sourceMappingURL=claude-skills.provider.js.map
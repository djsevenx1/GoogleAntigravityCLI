...
import { createNormalizedMessage, generateMessageId, readObjectRecord, sliceTailPage, truncateSubagentActivity, } from '../../../../shared/utils.js';
 ...
 */
async function readClaudeSubagentTranscript(filePath) {
    const activity = [];
    const transcript = { activity, endedMidToolCall: false };
...
async function readClaudeSubagentMeta(metaPath) {
    try {
 ...
 */
async function findClaudeSubagentTranscript(projectDirectory, providerSessionId, agentId) {
    const candidates = [
...
function readTaggedValue(content, tagName) {
 ...
 */
function collectTaskNotifications(messages) {
...
function readAgentToolUseId(message) {
    const content = message.message?.content;
...
function replaceAgentToolResultContent(message, replacement) {
    const content = message.message?.content;
    if (!Array.isArray(content)) {
        return;
    }
 ...
 */
async function readTranscriptRows(jsonlPath, providerSessionId) {
    const rows = [];
...
function isUserPromptRow(row) {
    ...
    if (Array.isArray(content)) {
        return content.some((part) => part?.type === 'text' || part?.type === 'image');
    }
 ...
 */
function dropSupersededPromptBranches(rows) {
    const promptSiblings = new Map();
...
async function getSessionMessages(sessionId, providerSessionId, limit, offset) {
    try {
        ...
        const messages = dropSupersededPromptBranches(await readTranscriptRows(jsonLPath, providerSessionId));
        ...
        const foldedNotificationUuids = new Set();
        for (const message of messages) {
            const agentId = message.toolUseResult?.agentId;
            if (!agentId) {
                continue;
 ...
 */
function parseLocalCommandPayload(content) {
 ...
 */
function buildLocalCommandDisplayText(payload) {
...
export class ClaudeSessionsProvider {
    ...
    normalizeMessageRows(rawMessage, sessionId) {
        const raw = readObjectRecord(rawMessage);
        if (!raw) {
            return [];
        }
            ...
            if (Array.isArray(raw.message.content)) {
                ...
                for (let partIndex = 0; partIndex < raw.message.content.length; partIndex++) {
                    ...
                    if (part.type === 'tool_result') {
                        ...
                        if ((parsedFiles.text || parsedFiles.attachments.length > 0)
    ...
    async resolveEditAnchor(sessionId, anchorId) {
        ...
        let parentUuid = target.parentUuid;

...
import { buildDefaultProviderCurrentActiveModel, getOpenCodeDatabasePath, readObjectRecord, readOptionalString, } from '../../../../shared/utils.js';
/**
 ...
 */
export const OPENCODE_PREDEFINED_MODELS = {
    OPTIONS: [
...
const OPENCODE_ENV_PROVIDER_IDS = {
    OPENCODE_API_KEY: 'opencode',
    ANTHROPIC_API_KEY: 'anthropic',
    OPENAI_API_KEY: 'openai',
};
const readOpenCodeJsonFile = async (filePath) => {
    try {
...
const readConnectedOpenCodeProviderIds = async () => {
    ...
    const auth = await readOpenCodeJsonFile(path.join(os.homedir(), '.local', 'share', 'opencode', 'auth.json'));
 ...
 */
const filterOpenCodeModelsByProvider = (definition, connectedProviderIds) => {
    if (!connectedProviderIds) {
        return definition;
    }
    const options = definition.OPTIONS.filter((option) => connectedProviderIds.has(option.value.split('/')[0]));
    if (options.length === 0) {
        return definition;
    }
    return {
...
const parseOpenCodeSessionModelValue = (rawModel) => {
...
export class OpenCodeProviderModels {
    async getSupportedModels() {
        return filterOpenCodeModelsByProvider(OPENCODE_PREDEFINED_MODELS, await readConnectedOpenCodeProviderIds());
    }
    async getCurrentActiveModel(sessionId) {
        if (!sessionId?.trim()) {
...
//# sourceMappingURL=opencode-models.provider.js.map
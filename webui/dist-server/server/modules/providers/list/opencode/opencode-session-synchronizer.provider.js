...
import { getOpenCodeDatabasePath, normalizeProviderTimestamp, normalizeSessionName, readJsonRecord, readOptionalString, unwrapJsonStringLiteral, } from '../../../../shared/utils.js';
/**
 * Session indexer for OpenCode's SQLite-backed session store.
 */
export class OpenCodeSessionSynchronizer {
    provider = 'opencode';
    /**
    ...
    upsertSession(db, row) {
        ...
        return sessionsDb.createSession(sessionId, this.provider, projectPath, normalizeSessionName(nextName, fallbackTitle), normalizeProviderTimestamp(row.time_created), normalizeProviderTimestamp(row.time_updated ?? row.time_created), null);
    }
    readFirstUserText(db, sessionId) {
        try {
...
//# sourceMappingURL=opencode-session-synchronizer.provider.js.map
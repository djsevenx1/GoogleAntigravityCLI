...
import { opencodeRuntime, resolveOpenCodePermissionOptions, } from './opencode-runtime.provider.js';
...
const findEnvKey = (name) => Object.keys(process.env).find((key) => key.toLowerCase() === name.toLowerCase()) || name;
...
test('spawnOpenCode emits session_created before normalized live messages for new sessions', async () => {
    ...
    try {
        ...
        const assistantDeltaIndex = messages.findIndex((message) => message.kind === 'stream_delta' && message.content === 'assistant response');
...
test('spawnOpenCode passes permission mode flags and env to the CLI', async () => {
    ...
    try {
        ...
        for (const scenario of scenarios) {
            ...
            process.env.OPENCODE_ARGS_CAPTURE = argsCapturePath;
            await opencodeRuntime.run('Hi', { cwd: tempRoot, permissionMode: scenario.permissionMode }, writer, runtimeContext);
            const capture = JSON.parse(await readFile(argsCapturePath, 'utf8'));
            for (const expectedArg of scenario.expectArgs) {
                assert.ok(capture.args.includes(expectedArg), `${scenario.permissionMode}: expected "${expectedArg}" in ${JSON.stringify(capture.args)}`);
            }
...
//# sourceMappingURL=opencode-runtime.provider.test.js.map
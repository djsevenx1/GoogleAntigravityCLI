...
import { AppError, createNormalizedMessage, generateMessageId, readObjectRecord, sliceTailPage, truncateSubagentActivity, } from '../../../../shared/utils.js';
        ...
        observe(entryType, payload) {
            if (entryType === 'event_msg' && payload.type === 'thread_rolled_back') {
                ...
                for (let retired = 0; retired < retiredTurns && liveTurnIds.length > 0; retired++) {
                    rolledBackTurnIds.add(liveTurnIds.pop());
                }
 ...
 * Exported for tests.
 */
export function extractCodexUserImages(payload) {
    if (!payload) {
        return undefined;
    }
    const candidates = [
...
function extractCodexTypedUserImages(content) {
    if (!Array.isArray(content)) {
        return undefined;
    }
    const attachments = [];
 ...
 * Exported for tests.
 */
export function readCodexProposedPlan(text) {
 ...
 * Exported for tests.
 */
export function readCodexMemoryCitations(text) {
...
function readStringProperty(objectSource, key) {
    const pattern = new RegExp(`(?:["']${key}["']|\\b${key})\\s*:\\s*("(?:\\\\.|[^"\\\\])*"|'(?:\\\\.|[^'\\\\])*'|\`(?:\\\\.|[^\`\\\\])*\`)`, 's');
...
function readSearchQueries(objectSource) {
 ...
 */
function readPatchLiteral(source) {
    const pattern = /("(?:\\.|[^"\\])*"|'(?:\\.|[^'\\])*'|`(?:\\.|[^`\\])*`)/gs;
 ...
 */
function readPlanSteps(objectSource) {
    const todos = [];
 ...
 */
function readMappedCommandCandidates(source) {
    const commands = [];
 ...
 * Exported for tests.
 */
export function parseCodexExecScript(input) {
 ...
 * Exported for tests.
 */
export function cleanCodexShellOutput(rawOutput) {
 ...
 */
function readCodexStdinOutput(rawOutput) {
 ...
 * Exported for tests.
 */
export function unifiedDiffToTexts(unifiedDiff) {
    const oldLines = [];
    const newLines = [];
 ...
 * Exported for tests.
 */
export function parseCodexPatch(patch) {
    const changes = [];
    ...
    for (const line of lines) {
        ...
        if (fileHeader) {
            flush();
            const changeType = fileHeader[1].toLowerCase();
...
function readPatchApplyChanges(changes) {
    ...
    for (const [filePath, rawChange] of Object.entries(record)) {
        ...
        const { oldText, newText } = unifiedDiffToTexts(typeof change.unified_diff === 'string' ? change.unified_diff : '');
        parsed.push({
            filePath,
            changeType,
...
function fileChangeToToolMessage(change, timestamp, toolCallId) {
...
function readRowFilePath(row) {
    try {
        return String(JSON.parse(String(row.toolInput ?? '{}')).file_path ?? '');
    }
    catch {
        return '';
    }
 ...
 */
function rewritePatchRows(rows, changes, callId, messages) {
...
function parseCodexSubagentMessage(payload) {
 ...
 */
async function findCodexSubagentRollout(parentFilePath, agentThreadId) {
...
async function findFileWithSuffix(directory, suffix, depth) {
    let entries;
    try {
 ...
 */
async function readCodexSubagentTranscript(filePath) {
    const activity = [];
 ...
 */
function translateCodexToolCall(payload, callId) {
    ...
    if (name === 'shell_command' || name === 'exec_command') {
        let command = '';
        try {
            const args = JSON.parse(String(rawInput || '{}'));
 ...
 */
function readCodexPlanInput(rawArguments) {
    try {
...
function humanizeCodexToolName(toolName) {
    return toolName
        .replace(/__/g, ' ')
        .replace(/_/g, ' ')
 ...
 */
async function getCodexSessionMessages(sessionId) {
    ...
    const shellCallMessages = new Map();
    /**
    ...
    let pendingPatchCallId = null;
    const execCallByCellId = new Map();
    const waitCallToExecCall = new Map();
    const pendingExecOutput = new Map();
    const completedExecCalls = new Set();
    /** Newest still-running exec call, for `write_stdin` polls that carry no cell id. */
    let runningExecCallId = null;
    const subagentsByCallId = new Map();
    const subagentsByPath = new Map();
    const turns = createCodexTurnTracker();
        ...
        if (entry.type === 'event_msg') {
            ...
            if (payload.type === 'patch_apply_end') {
                ...
                if (group) {
                    ...
                    const rows = changes.map((change, index) => fileChangeToToolMessage(change, timestamp, `${callId}#${index}`));
            ...
            if (payload.type === 'item_completed') {
                ...
                if (item?.type === 'UserMessage') {
                    ...
                    if (content.trim() || images?.length) {
                        ...
                        const isFirstPromptOfTurn = Boolean(turnId) && !anchoredTurnIds.has(turnId);
        ...
        if (payload.type === 'function_call' || payload.type === 'custom_tool_call') {
            ...
            if (name === 'exec') {
                ...
                if (patches.length > 0) {
                    ...
                    const rows = changes.map((change, index) => fileChangeToToolMessage(change, timestamp, index === 0 ? nextRowId() : `${callId}#${index}`));
                    rows.forEach((row) => messages.push(row));
        ...
        if (payload.type === 'function_call_output' || payload.type === 'custom_tool_call_output') {
            ...
            if (continuedCallId) {
                ...
                if (!cleaned.runningCellId) {
                    ...
                    pushToolResult(continuedCallId, timestamp, accumulated, cleaned.exitCode !== undefined && cleaned.exitCode !== 0);
                }
                continue;
            }
            ...
            if (subagent) {
                try {
                    const taskPath = readNonEmptyString(JSON.parse(rawOutput).task_name);
                    if (taskPath) {
            ...
            if (shellCallMessages.has(callId)) {
                ...
                pushToolResult(callId, timestamp, accumulated, cleaned.exitCode !== undefined && cleaned.exitCode !== 0);
                continue;
            }
 ...
 */
async function attachCodexSubagentTranscripts(parentFilePath, subagentsByCallId) {
    for (const record of subagentsByCallId.values()) {
        let parsedInput = {};
        try {
            parsedInput = JSON.parse(String(record.message.toolInput || '{}'));
        }
        catch {
            parsedInput = {};
        }
        ...
        const subagent = {
            ...
            description: readNonEmptyString(parsedInput.description) ?? agentName,
...
export class CodexSessionsProvider {
     ...
     */
    async resolveEditAnchor(sessionId, anchorId) {
     ...
     */
    async rewindSession(sessionId, keepThroughId) {
        const session = sessionsDb.getSessionById(sessionId);
     ...
     */
    normalizeHistoryEntry(raw, sessionId) {
    ...
    normalizeMessage(rawMessage, sessionId) {
        ...
        if (raw.type === 'item') {
            ...
            const itemId = readNonEmptyString(raw.itemId) ?? baseId;
            switch (raw.itemType) {
                case 'agent_message': {
                ...
                case 'file_change': {
                    ...
                    return changes.map((change, index) => createNormalizedMessage({
                        id: `${itemId}_${index}`,
                        sessionId,
                        timestamp: ts,
                        provider: PROVIDER,
                ...
                case 'mcp_tool_call': {
                    ...
                    return [toolUse, createNormalizedMessage({
                            ...
                            toolId: itemId,
                            content: raw.error
                                ? String(raw.error?.message ?? raw.error)
                ...
                case 'todo_list':
                    ...
                    return [createNormalizedMessage({
                            ...
                            toolName: 'TodoWrite',
                            toolInput: {
                                todos: (Array.isArray(raw.items) ? raw.items : []).map((item) => ({
...
//# sourceMappingURL=codex-sessions.provider.js.map
...
import { createNormalizedMessage, generateMessageId, sliceTailPage, } from '../../../../shared/utils.js';
...
function getTranscriptLines(logsDir) {
...
export class AntigravitySessionsProvider {
    ...
    async fetchHistory(sessionId, options) {
        ...
        const logsDir = path.join(resolveAntigravityStateDir(), 'brain', providerSessionId, '.system_generated', 'logs');
        let messages = [];
                ...
                for (const line of lines) {
                    try {
                        ...
                        if (source === 'USER_EXPLICIT' || stepType === 'USER_INPUT') {
                            ...
                            const isError = Boolean(entry.status === 'ERROR' ||
                                entry.error ||
            ...
            for (const line of lines) {
                if (line.includes('USER_SETTINGS_CHANGE') && line.includes('Model Selection')) {
                    ...
                    if (mMatch) {
                        ...
                        if (label.includes('3.8') && label.includes('Flash'))
                            detectedModel = 'gemini-3.8-flash';
                        else if (label.includes('3.7') && label.includes('Flash'))
                            detectedModel = 'gemini-3.7-flash';
                        else if (label.includes('3.6') && label.includes('Flash'))
                            detectedModel = 'gemini-3.6-flash';
                        else if (label.includes('3.1') && label.includes('Pro'))
                            detectedModel = 'gemini-3.1-pro';
                        else if (label.includes('Sonnet 4.6'))
                            detectedModel = 'claude-sonnet-4-6';
                        else if (label.includes('Opus 4.6'))
                            detectedModel = 'claude-opus-4-6-thinking';
                        else if (label.includes('120B'))
                            detectedModel = 'gpt-oss-120b-medium';
                        break;
                    }
                }
            }
        ...
        if (lastAssistant) {
            try {
                ...
                const liveSnapshot = antigravityAccountsService.getLiveQuotaCached() ||
                ...
                if (snapshotWithModel) {
                    lastAssistant.quotaSnapshot = snapshotWithModel;
                    lastAssistant.model = detectedModel;
                    lastAssistant.meta = {
                        ...lastAssistant.meta,
...
//# sourceMappingURL=antigravity-sessions.provider.js.map
...
import { extractFirstValidJsonlData, findFilesRecursivelyCreatedAfter, normalizeSessionName, readFileTimestamps, } from '../../../../shared/utils.js';
/**
 * Returns directory entries or an empty list when the folder is missing.
 */
async function listDirectoryEntriesSafe(directoryPath) {
    try {
...
export class CursorSessionSynchronizer {
    provider = 'cursor';
    cursorHome = path.join(os.homedir(), '.cursor');
    /**
     ...
     */
    async extractProjectPathFromWorkerLog(filePath) {
        try {
...
//# sourceMappingURL=cursor-session-synchronizer.provider.js.map
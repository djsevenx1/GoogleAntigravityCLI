...
import { findProviderSkillMarkdownFiles, readOptionalString, readProviderSkillMarkdownDefinitionFromContent, readProviderSkillMarkdownDefinition, AppError, } from '../../../../shared/utils.js';
...
const normalizeSkillDirectoryName = (value) => (value
    .trim()
    .replace(/[\\/]+/g, '-')
    .replace(/[<>:"|?*\x00-\x1F]/g, '-')
    .replace(/\s+/g, '-')
    ...
    .replace(/^\.+|\.+$/g, '')
    .replace(/^-+|-+$/g, ''));
const resolveSkillSupportingFilePath = (skillDirectoryPath, relativePath, entryIndex) => {
    ...
    if (!normalizedRelativePath
        ...
        throw new AppError(`Skill entry ${entryIndex + 1} includes an invalid supporting file path "${relativePath}".`, {
    ...
    if (!resolvedFilePath.startsWith(`${resolvedSkillDirectoryPath}${path.sep}`)) {
        throw new AppError(`Skill entry ${entryIndex + 1} supporting files must stay inside the skill directory.`, {
            code: 'PROVIDER_SKILL_FILE_PATH_INVALID',
            statusCode: 400,
        });
    }
...
export class SkillsProvider {
    ...
    async addSkills(input) {
        ...
        for (const [index, entry] of input.entries.entries()) {
            ...
            const fallbackSkillName = normalizeSkillDirectoryName(requestedDirectoryName
            ...
            const resolvedDirectoryName = normalizeSkillDirectoryName(requestedDirectoryName ?? definition.name);
    ...
    async removeSkill(input) {
        ...
        const resolvedSkillDirectoryPath = path.resolve(skillDirectoryPath);
        if (resolvedSkillDirectoryPath !== resolvedRoot

import { APP_CONFIG_TABLE_SCHEMA_SQL, LAST_SCANNED_AT_SQL, NOTIFICATION_CHANNEL_ENDPOINTS_TABLE_SCHEMA_SQL, PROJECTS_TABLE_SCHEMA_SQL, PROVIDER_MODELS_TABLE_SCHEMA_SQL, PUSH_SUBSCRIPTIONS_TABLE_SCHEMA_SQL, SESSION_DRAFTS_TABLE_SCHEMA_SQL, SUPERSEDED_PROVIDER_SESSIONS_TABLE_SCHEMA_SQL, SCHEDULED_MESSAGES_TABLE_SCHEMA_SQL, SESSIONS_TABLE_SCHEMA_SQL, USER_PREFERENCES_TABLE_SCHEMA_SQL, USER_NOTIFICATION_PREFERENCES_TABLE_SCHEMA_SQL, VAPID_KEYS_TABLE_SCHEMA_SQL, } from '../../modules/database/schema.js';
 ...
 */
const addProviderSessionIdMapping = (db) => {
    const sessionsTableInfo = getTableInfo(db, 'sessions');
    const columnNames = sessionsTableInfo.map((column) => column.name);
 ...
 */
const addSupersededTranscriptPathColumn = (db) => {
...
const addForkedFromSessionIdColumn = (db) => {
 ...
 */
const addSessionModelColumn = (db) => {
    const sessionsTableInfo = getTableInfo(db, 'sessions');
    const columnNames = sessionsTableInfo.map((column) => column.name);
 ...
 */
const addSessionEffortColumn = (db) => {
    const sessionsTableInfo = getTableInfo(db, 'sessions');
    const columnNames = sessionsTableInfo.map((column) => column.name);

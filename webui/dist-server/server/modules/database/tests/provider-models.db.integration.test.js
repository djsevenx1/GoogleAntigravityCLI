...
import { closeConnection, getConnection, initializeDatabase, providerModelsDb, sessionsDb, } from '../../../modules/database/index.js';
...
test('provider model repository stores custom rows only and maintains session references', async () => {
    ...
    try {
        const columns = getConnection().prepare('PRAGMA table_info(provider_models)').all();
        ...
        assert.equal(providerModelsDb.findCustomProviderModelByModelId('codex', 'gateway/model-v1')?.recordId, custom.recordId);
    ...
    `).run();
        ...
        const removed = providerModelsDb.deleteCustomProviderModel('codex', custom.recordId, 'gpt-default');
...
//# sourceMappingURL=provider-models.db.integration.test.js.map
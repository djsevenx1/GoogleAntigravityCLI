...
import { closeConnection, getConnection, initializeDatabase, sessionDraftsDb, userPreferencesDb, } from '../../../modules/database/index.js';
const USER_ID = 1;
async function withDatabase(runTest) {
...
test('one unreadable preference value does not cost the user the others', async () => {
    await withDatabase(() => {
        ...
        getConnection()
            .prepare(`INSERT INTO user_preferences (user_id, preference_key, preference_value)
         VALUES (?, ?, ?)`)
...
test('deleteDraft removes only the named scope', async () => {
    await withDatabase(() => {
        ...
        assert.deepEqual(sessionDraftsDb.getDrafts(USER_ID).map((draft) => draft.scope), ['session-b']);
    });
});
//# sourceMappingURL=user-preferences.db.integration.test.js.map
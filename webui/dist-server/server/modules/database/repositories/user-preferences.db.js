 ...
 */
function decodePreference(row) {
    try {
        return [row.preference_key, JSON.parse(row.preference_value)];
    }
    catch {
...
export const userPreferencesDb = {
    ...
    getPreferences(userId) {
        ...
        const rows = db
            .prepare(`SELECT preference_key, preference_value
    ...
    savePreferences(userId, updates) {
        const db = getConnection();
        const upsert = db.prepare(`INSERT INTO user_preferences (user_id, preference_key, preference_value, updated_at)
       VALUES (?, ?, ?, CURRENT_TIMESTAMP)
       ON CONFLICT(user_id, preference_key) DO UPDATE SET
         preference_value = excluded.preference_value,
         updated_at = CURRENT_TIMESTAMP`);
        const remove = db.prepare('DELETE FROM user_preferences WHERE user_id = ? AND preference_key = ?');
        db.transaction(() => {
...
//# sourceMappingURL=user-preferences.db.js.map
...
function parseQueuedMessage(raw) {
    if (!raw) {
        return null;
    }
    try {
...
export const sessionDraftsDb = {
    ...
    getDrafts(userId) {
        ...
        const rows = db
            .prepare(`SELECT draft_scope, draft_text, queued_message, updated_at
         FROM session_drafts
         WHERE user_id = ?
    ...
    listQueuedMessages() {
        const rows = getConnection()
            .prepare(`SELECT drafts.user_id, drafts.draft_scope, drafts.queued_message
    ...
    claimQueuedMessage(candidate) {
        const result = getConnection()
            .prepare(`UPDATE session_drafts
    ...
    restoreQueuedMessage(candidate) {
        getConnection()
            .prepare(`UPDATE session_drafts
    ...
    deleteEmptyDraft(userId, scope) {
        getConnection()
            .prepare(`DELETE FROM session_drafts
    ...
    saveDraft(userId, scope, draft) {
        ...
        db.prepare(`INSERT INTO session_drafts (user_id, draft_scope, draft_text, queued_message, updated_at)
       VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP)
       ON CONFLICT(user_id, draft_scope) DO UPDATE SET
         draft_text = excluded.draft_text,
         queued_message = excluded.queued_message,
         updated_at = CURRENT_TIMESTAMP`).run(userId, scope, draft.text, draft.queuedMessage === null ? null : JSON.stringify(draft.queuedMessage));
    },
    deleteDraft(userId, scope) {
        const db = getConnection();
        db.prepare('DELETE FROM session_drafts WHERE user_id = ? AND draft_scope = ?')
            .run(userId, scope);
    },
};
//# sourceMappingURL=session-drafts.db.js.map
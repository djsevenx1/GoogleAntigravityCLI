...
import { buildStoredAttachmentRecords, buildStoredImageRecords, isAllowedImageMimeType, resolveAttachmentAssetFile, resolveImageAssetFile, } from '../../../modules/assets/services/image-assets.service.js';
...
test('resolveAttachmentAssetFile uses the same direct-child boundary', () => {
    assert.equal(resolveAttachmentAssetFile('123-notes.txt'), path.join(path.resolve(ASSETS_DIR), '123-notes.txt'));
    assert.equal(resolveAttachmentAssetFile('../notes.txt'), null);
});
//# sourceMappingURL=image-assets.service.test.js.map
...
function parseSandboxArguments(argumentsList, homeDirectory) {
    const parsedOptions = {
        subcommand: null,
        workspace: null,
        agent: 'claude',
    ...
    for (let argumentIndex = 0; argumentIndex < argumentsList.length; argumentIndex += 1) {
        ...
        if (argumentIndex === 0 && subcommands.includes(argument)) {
            parsedOptions.subcommand = argument;
        }
...
function showSandboxHelp(output) {
    output.log(`
...
function requireSandboxName(options, output) {
    if (options.name) {
        return options.name;
    }
...
function publishSandboxPort(options, dependencies) {
    const sandboxName = options.name;
    dependencies.output.log(`${terminalTextStyles.info('▶')} Forwarding port ${options.port} → 3001...`);
    try {
        dependencies.runSandboxCommand(['ports', sandboxName, '--publish', `${options.port}:3001`]);
        return true;
    }
    catch (error) {
        ...
        dependencies.output.log(`${terminalTextStyles.warn('⚠')}  Port ${options.port} in use, trying ${alternativePort}...`);
        try {
            dependencies.runSandboxCommand(['ports', sandboxName, '--publish', `${alternativePort}:3001`]);
            options.port = alternativePort;
            return true;
        }
        catch {
            dependencies.output.error(`${terminalTextStyles.error('❌')} Ports ${options.port} and ${alternativePort} both in use. Use --port to specify a free port.`);
            return false;
        }
    }
}
 ...
 */
export function createSandboxCommandService(dependencies) {
    return {
        async execute(argumentsList) {
            ...
            switch (options.subcommand) {
                ...
                case 'logs': {
                    ...
                    try {
                        dependencies.runSandboxCommand(['exec', sandboxName, 'bash', '-c', 'cat /tmp/cloudcli-ui.log'], true);
                    }
                    catch (error) {
                ...
                case 'create': {
                    ...
                    if (options.environmentVariables.length > 0) {
                        ...
                        if (invalidEnvironmentVariables.length > 0) {
                            dependencies.output.log(`${terminalTextStyles.warn('⚠')}  Skipped invalid env vars: ${invalidEnvironmentVariables.join(', ')} (expected KEY=VALUE)`);
                        }
                    }
                    dependencies.output.log(`${terminalTextStyles.info('▶')} Launching CloudCLI web server...`);
                    dependencies.runSandboxCommand([

...
function sourceModuleUrl(...segments) {
    return pathToFileURL(path.join(applicationRoot, ...segments)).href;
}
async function runCliBootstrapFixture(isPlatform) {
    ...
    try {
        ...
        await Promise.all([
            copyFile(path.join(applicationRoot, 'server', 'load-env.ts'), path.join(fixtureServerDirectory, 'load-env.ts')),
            copyFile(path.join(applicationRoot, 'server', 'modules', 'cli', 'cli.ts'), path.join(fixtureCliDirectory, 'cli.ts')),
        ]);
        await writeFile(path.join(fixtureRoot, '.env'), [
...
//# sourceMappingURL=cli-environment-bootstrap.test.js.map
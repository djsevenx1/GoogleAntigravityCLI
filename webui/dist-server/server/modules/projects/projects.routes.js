...
router.get('/', asyncHandler(async (req, res) => {
    // 性能优化：默认直接从数据库极速读取(耗时2ms)，避免全盘扫描300+会话导致页面卡顿80秒
    // 只有在明确传入 sync=1 或 forceSync=1 时才进行阻塞同步
    const forceSync = readQueryStringValue(req.query.sync).trim() === '1' ||
        readQueryStringValue(req.query.forceSync).trim() === '1' ||
        readQueryStringValue(req.query.skipSync).trim() === '0' ||
        readQueryStringValue(req.query.skipSynchronization).trim() === '0';
    const skipSynchronization = !forceSync;

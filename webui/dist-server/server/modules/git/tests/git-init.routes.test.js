...
test('git init does not run when repository validation fails for an execution error', async () => {
    ...
    const unexpectedProvider = async () => { throw new Error('unexpected provider call'); };
    const router = createGitRouter({
        fileSystem: { access: async () => undefined },
        spawnProcess,
    ...
    const spawnProcess = ((_command, args) => {
        ...
        process.nextTick(() => {
            ...
            if (args.includes('--show-toplevel'))
                child.stdout.write('/workspace/repo\n');
            if (args.includes('--show-current'))
                child.stdout.write('main\n');
            if (args.includes('-D'))
                child.stdout.write('Deleted branch feature/unmerged.\n');
            child.stdout.end();
            child.stderr.end();
            child.emit('close', 0);
        });
...
//# sourceMappingURL=git-init.routes.test.js.map
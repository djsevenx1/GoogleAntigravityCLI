...
export function createGitRouter(dependencies) {
    ...
    router.post('/generate-commit-message', async (req, res) => {
        const { project, files, provider = 'antigravity' } = req.body;
        if (!project || !files || files.length === 0) {
            return res.status(400).json({ error: 'Project id and files are required' });
        }
        // Validate provider
        if (provider !== 'antigravity' && !['claude', 'cursor'].includes(provider)) {
...
Generate the commit message:`;
        try {
            ...
            if (provider === 'antigravity' && queryAntigravity) {
                await queryAntigravity(prompt, {
                    cwd: projectPath,
                }, writer);
            }
            else if (provider === 'claude' && queryClaudeSDK) {
            ...
            else if (provider === 'cursor' && spawnCursor) {

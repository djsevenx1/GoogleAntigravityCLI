...
export async function deleteLocalBranch(input) {
    const { stdout: currentBranch } = await input.runCommand('git', ['branch', '--show-current'], { cwd: input.projectPath });
    ...
    const { stdout } = await input.runCommand('git', ['branch', deleteFlag, '--', input.branch], { cwd: input.projectPath });
    return stdout;
}

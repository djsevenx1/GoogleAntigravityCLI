...
router.get('/status', async (_req, res) => {
    try {
        ...
        sendError(res, 500, BROWSER_USE_ERROR_CODES.STATUS_LOAD_FAILED, 'Failed to load Browser status.', error instanceof Error ? error.message : undefined);
    }
});
router.get('/settings', async (_req, res) => {
    try {
        ...
        sendError(res, 500, BROWSER_USE_ERROR_CODES.SETTINGS_LOAD_FAILED, 'Failed to load Browser settings.', error instanceof Error ? error.message : undefined);
    }
});
router.put('/settings', async (req, res) => {
    try {
        ...
        sendError(res, 400, BROWSER_USE_ERROR_CODES.SETTINGS_SAVE_FAILED, 'Failed to save Browser settings.', error instanceof Error ? error.message : undefined);
    }
});
router.post('/runtime/install', async (_req, res) => {
    try {
        ...
        sendError(res, 500, BROWSER_USE_ERROR_CODES.RUNTIME_INSTALL_FAILED, 'Failed to install Browser runtime.', result.message);
    }
    catch (error) {
        sendError(res, 500, BROWSER_USE_ERROR_CODES.RUNTIME_INSTALL_FAILED, 'Failed to install Browser runtime.', error instanceof Error ? error.message : undefined);
    }
});
router.get('/sessions', async (_req, res) => {
    try {
        ...
        sendError(res, 401, BROWSER_USE_ERROR_CODES.SESSIONS_LOAD_FAILED, 'Failed to list browser sessions.', error instanceof Error ? error.message : undefined);
    }
});
router.post('/sessions/:sessionId/stop', async (req, res) => {
    try {
        ...
        sendError(res, 400, BROWSER_USE_ERROR_CODES.SESSION_STOP_FAILED, 'Failed to stop browser session.', error instanceof Error ? error.message : undefined);
    }
});
router.delete('/sessions/:sessionId', async (req, res) => {
    try {
        ...
        sendError(res, 400, BROWSER_USE_ERROR_CODES.SESSION_DELETE_FAILED, 'Failed to delete browser session.', error instanceof Error ? error.message : undefined);
    }
});
export default router;
//# sourceMappingURL=browser-use.routes.js.map
...
function getSetupMessage(settings, readiness) {
    if (!settings.enabled) {
...
async function removeMcpServerFromAllProviders(name) {
...
function getRuntimeReadiness(options = {}) {
    const now = Date.now();
...
const INSTALL_COMMAND_TIMEOUT_MS = Number.parseInt(process.env.CLOUDCLI_BROWSER_USE_INSTALL_TIMEOUT_MS || String(10 * 60 * 1000), 10);
function runCommand(command, args) {
    return new Promise((resolve, reject) => {
        ...
        const timer = setTimeout(() => {
            child.kill('SIGKILL');
            finish(() => reject(new Error(`${command} ${args.join(' ')} timed out after ${INSTALL_COMMAND_TIMEOUT_MS}ms.`)));
...
function ownerSessions(ownerId) {
    return [...sessions.values()].filter((session) => session.ownerId === ownerId);
}
async function closeHandle(sessionId) {
    const handle = handles.get(sessionId);
    handles.delete(sessionId);
    await handle?.context?.close?.().catch(() => undefined);
    await handle?.browser?.close().catch(() => undefined);
}
async function expireStaleSessions(now = Date.now()) {
...
async function captureSession(session, page) {
...
async function getActionPoint(page, input) {
...
export const browserUseService = {
    ...
    async unregisterAgentMcp() {
        const results = (await Promise.all([MCP_SERVER_NAME, ...LEGACY_MCP_SERVER_NAMES].map((name) => removeMcpServerFromAllProviders(name)))).flat();
    ...
    async agentNavigate(sessionId, rawUrl) {
    ...
    async agentClick(sessionId, input) {
        const session = await this.getAgentSession(sessionId);
        const handle = handles.get(sessionId);
        if (!handle?.page) {
            throw new Error('Browser runtime handle is not available.');
    ...
    async agentType(sessionId, input) {
        const session = await this.getAgentSession(sessionId);
        const handle = handles.get(sessionId);
        if (!handle?.page) {
            throw new Error('Browser runtime handle is not available.');
        ...
        if (input.selector) {
            ...
            session.cursor = await getActionPoint(handle.page, input).then((point) => (point ? { ...point, actor: 'agent' } : null));
        }
        else {
    ...
    async agentFillForm(sessionId, fields) {
        const session = await this.getAgentSession(sessionId);
        const handle = handles.get(sessionId);
        if (!handle?.page) {
            throw new Error('Browser runtime handle is not available.');
        ...
        if (fields[0]) {
            session.cursor = await getActionPoint(handle.page, { selector: fields[0].selector }).then((point) => (point ? { ...point, actor: 'agent' } : null));
        }
        await captureSession(session, handle.page);
        return publicSession(session);
    },
    async agentPressKey(sessionId, key) {
        const session = await this.getAgentSession(sessionId);
        const handle = handles.get(sessionId);
        if (!handle?.page) {
            throw new Error('Browser runtime handle is not available.');
    ...
    async agentSelectOption(sessionId, selector, values) {
        const session = await this.getAgentSession(sessionId);
        const handle = handles.get(sessionId);
        if (!handle?.page) {
            throw new Error('Browser runtime handle is not available.');
        ...
        session.cursor = await getActionPoint(handle.page, { selector }).then((point) => (point ? { ...point, actor: 'agent' } : null));
        await captureSession(session, handle.page);
        return publicSession(session);
    },
    async agentWaitFor(sessionId, input) {
        const session = await this.getAgentSession(sessionId);
        const handle = handles.get(sessionId);
        if (!handle?.page) {
            throw new Error('Browser runtime handle is not available.');
    ...
    async agentTabs(sessionId, input) {
        const session = await this.getAgentSession(sessionId);
        const handle = handles.get(sessionId);
    ...
    async agentStopSession(sessionId) {
        await this.getAgentSession(sessionId);
...
//# sourceMappingURL=browser-use.service.js.map
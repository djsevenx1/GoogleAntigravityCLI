...
function createDirectoryEntry(name, directory) {
    return {
        name,
...
function createFakeFileSystem(overrides = {}) {
...
function createDependencies(fileSystem, projectRoot) {
    return {
        fileSystem,
        projects: {
...
test('listProjectFiles applies gitignore alongside hard directory exclusions', async () => {
    ...
    const fileSystem = createFakeFileSystem({
        ...
        lstat: async (candidatePath) => createStats(candidatePath === documentationDirectory
            || candidatePath === buildDocumentationDirectory
            || candidatePath === sourceDirectory
            || candidatePath === gitDirectory
            || candidatePath === nodeModulesDirectory, 0o754),
    });
...
test('listProjectFiles excludes gitignored entries only when requested', async () => {
    ...
    const fileSystem = createFakeFileSystem({
        ...
        lstat: async (candidatePath) => createStats(candidatePath === cacheDirectory || candidatePath === sourceDirectory, 0o644),
    });
    const service = createFileTreeService(createDependencies(fileSystem, projectRoot));
    const tree = await service.listProjectFiles('project-1', { respectGitignore: true });
...
test('listProjectFiles rejects a tree that exceeds the server entry limit', async () => {
    ...
    const service = createFileTreeService(createDependencies(fileSystem, projectRoot));
    await assert.rejects(service.listProjectFiles('project-1'), (error) => error instanceof AppError
    ...
    const fileSystem = createFakeFileSystem({
        ...
        openDirectory: createDirectoryReader((directoryPath) => {
            ...
            if (directoryPaths.has(directoryPath)) {
                return Array.from({ length: 5_000 }, (_, index) => createDirectoryEntry(`${path.basename(directoryPath)}-${index}.txt`, false));
            }
            return [];
        }),
...
test('readTextFile rejects traversal before invoking the filesystem adapter', async () => {
    ...
    const service = createFileTreeService(createDependencies(fileSystem, projectRoot));
    await assert.rejects(service.readTextFile('project-1', '../secret.txt'), (error) => error instanceof AppError
...
//# sourceMappingURL=file-tree.service.test.js.map
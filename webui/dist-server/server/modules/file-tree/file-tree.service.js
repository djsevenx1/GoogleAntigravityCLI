...
function permissionBitsToRwx(permissionBits) {
...
function expandWorkspacePath(workspaceRoot, inputPath) {
    if (inputPath === '~') {
...
function mapFileSystemError(error, messages) {
...
function cleanAndExtractCandidatePath(raw) {
    const candidates = [];
...
function createGitignoreEntryFilter(projectRoot, gitignoreContent) {
    const gitignore = ignore().add(gitignoreContent);
    return (entryPath, isDirectory) => {
        if (!includeEntryByHardExclusions(entryPath, isDirectory))
            return false;
 ...
 */
export function createFileTreeService(dependencies) {
     ...
     */
    async function collectVisibleEntries(directoryPath, includeEntry, remainingEntries) {
        ...
        try {
            for await (const entry of fileSystem.openDirectory(directoryPath)) {
                ...
                if (!isDirectory && entry.isSymbolicLink?.()) {
                    try {
                        const stat = await fileSystem.stat(path.join(directoryPath, entry.name));
                        if (stat.isDirectory()) {
                            isDirectory = true;
                        }
                    }
                ...
                if (remainingEntries.value <= 0) {
                    throw createFileTreeError(`Project file tree exceeds the ${MAXIMUM_FILE_TREE_ENTRIES.toLocaleString()} entry limit. Choose a narrower project directory or add ignore rules.`, 413, 'FILE_TREE_TOO_LARGE');
                }
    ...
    async function buildFileTree(directoryPath, maximumDepth, currentDepth = 0, includeEntry = includeEntryByFallbackDirectoryNames, remainingEntries = { value: MAXIMUM_FILE_TREE_ENTRIES }) {
        ...
        const items = await Promise.all(visibleEntries.map(async (entry) => {
            ...
            try {
                ...
                try {
                    ...
                    if (stats.isSymbolicLink()) {
                        item.isSymlink = true;
                        try {
                            const targetStat = await fileSystem.stat(itemPath);
            ...
            if (entry.isDirectory() && currentDepth < maximumDepth && !isForbiddenSystemDir) {
                item.children = await buildFileTree(itemPath, maximumDepth, currentDepth + 1, includeEntry, remainingEntries);
            }
            return item;
        }));
    ...
    return {
        async browseWorkspace(inputPath) {
            ...
            const isRoot = targetPath === '/' || targetPath === path.resolve('/');
            if (!isRoot) {
            ...
            let directories = fileTree
                .filter((item) => item.type === 'directory')
                .map((item) => ({ path: item.path, name: item.name, type: 'directory' }))
                .sort((left, right) => {
            ...
            // 如果在根目录浏览，过滤系统关键目录，只保留用户数据卷(如 vol1, vol5, home 等)
            if (isRoot) {
                const systemDirs = new Set(['etc', 'bin', 'sbin', 'usr', 'proc', 'sys', 'dev', 'boot', 'root', 'run', 'lib', 'lib64']);
                directories = directories.filter((dir) => !systemDirs.has(dir.name));
            }
            // 如果当前目录由于权限原因(例如 NAS 卷根路径权限限制)未枚举到任何子目录，尝试探测常见的合法子目录(如 @apphome 等)
            if (directories.length === 0) {
                const probeCandidates = ['@apphome', 'home', 'data', 'workspace', 'projects', 'docker', 'shares'];
                for (const candidate of probeCandidates) {
                    const candidatePath = path.join(resolvedPath, candidate);
                    try {
                        const candStat = await fileSystem.stat(candidatePath);
            ...
            return { path: resolvedPath, suggestions: directories };
        },
        ...
        async createEntry(input) {
            ...
            try {
                await fileSystem.access(resolvedPath);
                throw createFileTreeError(`${input.type === 'file' ? 'File' : 'Directory'} already exists`, 409, 'FILE_TREE_ENTRY_EXISTS');
            }
            catch (error) {
                if (error instanceof AppError)
                    throw error;
        ...
        async renameEntry(input) {
            ...
            try {
                ...
                throw createFileTreeError('A file or directory with this name already exists', 409, 'FILE_TREE_ENTRY_EXISTS');
            }
            catch (error) {
                if (error instanceof AppError)
                    throw error;

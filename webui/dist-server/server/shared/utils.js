 ...
 * across endpoints.
 */
export function createApiSuccessResponse(data) {
    return {
        success: true,
        data,
    };
...
function shouldUseWindowsPathNormalization(inputPath) {
    ...
    try {
        ...
        try {
            ...
            try {
                ...
                const parentFileError = parentError;
        ...
        // 存储卷与多工作区放行支持：
        // 当 WORKSPACES_ROOT 为 '/' 或启用了全局路径支持时，只要不属于系统危险黑名单，均允许作为工作区
        const allowAnyWorkspace = process.env.ALLOW_ALL_WORKSPACES === 'true' ||
            process.env.ALLOW_ANY_WORKSPACE_PATH === 'true' ||
            WORKSPACES_ROOT === '/' ||
            WORKSPACES_ROOT === '' ||
            process.env.ALLOW_ALL_WORKSPACES !== 'false';
        if (!allowAnyWorkspace) {
            const resolvedWorkspaceRoot = normalizeProjectPath(await realpath(WORKSPACES_ROOT));
            if (!resolvedPath.startsWith(`${resolvedWorkspaceRoot}${path.sep}`)
        ...
        try {
            ...
            if (pathStats.isSymbolicLink()) {
                ...
                if (!allowAnyWorkspace) {
                    const resolvedWorkspaceRoot = normalizeProjectPath(await realpath(WORKSPACES_ROOT));
                    if (!realSymlinkPath.startsWith(`${resolvedWorkspaceRoot}${path.sep}`)
 ...
 */
export function truncateSubagentActivity(activity) {
 ...
 */
export function sliceTailPage(items, limit, offset) {
 ...
 */
export const readObjectRecord = (value) => {
 ...
 * "not configured".
 */
export const readOptionalString = (value) => {
 ...
 */
export const readStringArray = (value) => {
 ...
 */
export const readStringRecord = (value) => {
 ...
 */
export function buildDefaultProviderCurrentActiveModel(models) {
    return {
 ...
 */
export const parseIncomingJsonObject = (payload) => {
    let text = null;
...
export const readJsonConfig = async (filePath) => {
    try {
        ...
        if (error instanceof SyntaxError) {
            console.warn(`[readJsonConfig] invalid or empty JSON in ${filePath}, treating as empty:`, error.message);
            return {};
        }
        throw error;
    }
 ...
 */
export const writeJsonConfig = async (filePath, data) => {
 ...
 */
export async function findTopmostGitRoot(startPath) {
 ...
 */
export function addUniqueProviderSkillSource(sources, seenRootDirs, source) {
 ...
 */
export async function findProviderSkillMarkdownFiles(rootDir, options = {}) {
    const skillFiles = [];
 ...
 */
export async function readProviderSkillMarkdownDefinition(skillPath) {
    const content = await readFile(skillPath, 'utf8');
    return readProviderSkillMarkdownDefinitionFromContent(content, path.basename(path.dirname(skillPath)));
}
/**
 ...
 */
export function readProviderSkillMarkdownDefinitionFromContent(content, fallbackName) {
 ...
 */
export function normalizeSessionName(rawValue, fallback) {
...
/**
 * Checks whether a session title is a generic provider-generated fallback
 * rather than a meaningful user request or custom name.
 */
export function isGenericSessionName(name) {
    if (!name)
        return true;
    const trimmed = name.trim();
    if (!trimmed)
        return true;
    return (/^Antigravity(\s+[\w-]+)?$/i.test(trimmed) ||
        /^Untitled\s+(Claude|Codex|Cursor|Session)/i.test(trimmed) ||
        /^OpenCode(\s+[\w-]+)?$/i.test(trimmed));
}
// ---------------------------
 ...
 */
export function readJsonRecord(value) {
    if (typeof value !== 'string') {
 ...
 */
export function unwrapJsonStringLiteral(value) {
 ...
 */
export function sanitizeLeafDirectoryName(inputName, label = 'directory name') {
 ...
 */
export async function findFilesRecursivelyCreatedAfter(rootDir, extension, lastScanAt, fileList = []) {
    try {
        const entries = await readdir(rootDir, { withFileTypes: true });
        for (const entry of entries) {
 ...
 */
export async function readFileTimestamps(filePath) {
    try {
 ...
 */
export async function buildLookupMap(filePath, keyField, valueField) {
 ...
 */
export async function extractFirstValidJsonlData(filePath, extractor) {
    try {
        const fileStream = fs.createReadStream(filePath);
        const lineReader = readline.createInterface({ input: fileStream, crlfDelay: Infinity });
        for await (const line of lineReader) {
 ...
 */
export function flattenPromptForWindowsShell(prompt) {
...
export const terminalTextStyles = {
    info: (text) => `${ANSI_TERMINAL_STYLES.cyan}${text}${ANSI_TERMINAL_STYLES.reset}`,
    ok: (text) => `${ANSI_TERMINAL_STYLES.green}${text}${ANSI_TERMINAL_STYLES.reset}`,
    warn: (text) => `${ANSI_TERMINAL_STYLES.yellow}${text}${ANSI_TERMINAL_STYLES.reset}`,
    error: (text) => `${ANSI_TERMINAL_STYLES.yellow}${text}${ANSI_TERMINAL_STYLES.reset}`,
    tip: (text) => `${ANSI_TERMINAL_STYLES.blue}${text}${ANSI_TERMINAL_STYLES.reset}`,
    bright: (text) => `${ANSI_TERMINAL_STYLES.bright}${text}${ANSI_TERMINAL_STYLES.reset}`,
    dim: (text) => `${ANSI_TERMINAL_STYLES.dim}${text}${ANSI_TERMINAL_STYLES.reset}`,
};
// ---------------------------
 ...
 */
export function getModuleDirectory(importMetaUrl) {
 ...
 */
export function findServerRoot(startDirectory) {

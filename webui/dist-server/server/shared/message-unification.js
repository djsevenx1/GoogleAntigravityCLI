 ...
 */
function readToolPayload(value) {
 ...
 */
function readAnswerLabels(value) {
    if (typeof value === 'string') {
        return value;
    }
 ...
 */
function readAnswersFromAcknowledgement(content) {
 ...
 */
function keyAnswersByQuestion(questions, rawAnswers) {
...
function readAskAnswers(message) {
...
class ChecklistState {
    ...
    upsertTask(task, defaults = { content: '', activeForm: '' }) {
        if (!task) {
            return;
        }
...
function isChecklistSnapshot(message) {
 ...
 */
function readChecklistSignature(message) {
 ...
 */
function truncateNestedOutput(value) {
    if (typeof value === 'string') {
...
function capToolResult(message) {
 ...
 */
export function prepareTranscriptMessages(messages) {
    ...
    // renders on its own.
    return messages.filter((message, index) => (!superseded.has(index)
        && !(message.kind === 'tool_result' && message.toolId)));
}
//# sourceMappingURL=message-unification.js.map
...
import { appendFilesInputTag, appendImagesInputTag, buildClaudeUserContent, buildCodexInputItems, isImageAttachmentDescriptor, normalizeAttachmentDescriptors, isAllowedImageSourcePath, normalizeImageDescriptors, parseFilesInputTag, parseImagesInputTag, resolveImageMediaType, toImageAttachments, } from '../../shared/image-attachments.js';
// 1x1 transparent PNG
...
async function createSymlinkIfSupported(target, linkPath, type) {
    try {
...
test('parseFilesInputTag handles Windows-flattened provider prompts', () => {
    const flattened = appendFilesInputTag('inspect this', [{ path: 'C:/Users/x/.cloudcli/assets/report.docx', name: 'report.docx' }]).replace(/\s*\r?\n\s*/g, ' ');
...
test('only the LAST images_input block is treated as the attachment carrier', () => {
    ...
    const tagged = appendImagesInputTag(`${userTypedTag}\n\n<images_input>\nfake user block\n</images_input>\n\nAlso check this.`, [{ path: 'C:/Users/x/.cloudcli/assets/real.png' }]);
    const parsed = parseImagesInputTag(tagged);
...
test('buildClaudeUserContent reads image bytes into base64 blocks', async () => {
    ...
    try {
        ...
        const content = await buildClaudeUserContent('What is in this image?', [{ path: 'shot.png', mimeType: 'image/png' }], tempDir);
...
test('buildClaudeUserContent skips unsupported types and unreadable files', async () => {
    ...
    try {
        ...
        const content = await buildClaudeUserContent('prompt', [
...
test('buildClaudeUserContent refuses symlinked images outside allowed roots', async (t) => {
    ...
    try {
        ...
        const content = await buildClaudeUserContent('prompt', [{ path: 'linked-secret.png', mimeType: 'image/png' }], tempDir);
        assert.deepEqual(content, [{ type: 'text', text: 'prompt' }]);
    }
    finally {
        await rm(tempDir, { recursive: true, force: true });
...
test('buildClaudeUserContent accepts images under a symlinked cwd', async (t) => {
    ...
    try {
        ...
        const content = await buildClaudeUserContent('prompt', [{ path: 'shot.png', mimeType: 'image/png' }], linkCwd);
        assert.equal(content.length, 2);
        assert.equal(content[1].type, 'image');
        const imageBlock = content[1];
        assert.equal(imageBlock.source.data, PNG_BYTES.toString('base64'));
...
test('provider builders refuse descriptors outside the allowed roots', async () => {
    ...
    const claudeContent = await buildClaudeUserContent('prompt', [{ path: outsidePath, mimeType: 'image/png' }], cwd);
    assert.deepEqual(claudeContent, [{ type: 'text', text: 'prompt' }]);
});
//# sourceMappingURL=image-attachments.test.js.map
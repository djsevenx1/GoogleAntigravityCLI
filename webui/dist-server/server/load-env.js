...
function getBootstrapApplicationRoot(importMetaUrl) {
    ...
    while (path.basename(serverRoot) !== 'server') {
        ...
        if (parent === serverRoot)
            throw new Error('Could not resolve server root');
        serverRoot = parent;
    }
    const parent = path.dirname(serverRoot);
...
if (process.env.AGY_HOME) {
    // AGY_HOME marks an isolated test instance: keep its conversation history in its
    // own database and override any DATABASE_PATH inherited from the shell profile,
    // so history and chats never leak into the shared instance's auth.db.
    process.env.DATABASE_PATH = path.join(process.env.AGY_HOME, '.cloudcli', 'auth.db');
}
